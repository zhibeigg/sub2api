...
GPT/OpenAI 原生订阅套餐会迁移到启用的 OpenAI 标准分组 `GPT 稳定分组 无限制`。迁移会将旧分组当前额度快照到套餐和有效用户订阅，保留额度窗口、已用额度和有效期，同步受影响 API Key、专属分组授权及受限分组访问，并清除用户专属倍率覆盖，使请求按目标分组当前默认、模型和高峰倍率计费。同一用户有多条有效旧 GPT 订阅时，会按各周期剩余额度求和合并为一条目标订阅、清零计数并重新开始额度窗口，同时保留最晚有效期。目标分组不存在/不唯一，或用户已有冲突的目标分组订阅时，迁移会在写入前安全失败。
