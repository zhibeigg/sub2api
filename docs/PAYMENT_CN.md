...
订阅套餐分为两类。`subscription` 套餐只能绑定一个订阅分组并沿用该分组自身限额；`standard_quota` 套餐可绑定一个或多个标准（余额）分组，并配置共享的日、周、月 USD 套餐额度，以及可选的订阅实例级 `concurrency_limit`。购买或管理员分配时会快照套餐类型、分组、额度、并发限制和有效期；后续修改套餐不会追溯影响已有订阅。管理员还可分别隐藏余额充值与订阅购买入口，后端会同步拒绝对应的新订单类型。
