...
- 管理后台白名单优先以 `GET /v1/models` 返回的逻辑模型 ID 为准；仅配置 Dashboard Token 的 IDE-only 账号会从 `AiService/AvailableModels` 的 `legacySlugs` 折叠出同样的逻辑目录。IDE 执行变体仅用于把逻辑模型路由到 thinking、effort、fast 等具体 `serverModelName`，不会逐项写入白名单。
...
服务端后台维护该独立会话：按配置周期检查凭据，在配置的提前刷新窗口内续期，并按较长周期主动探测 Dashboard 用量 RPC。刷新成功后会加密回写新凭据和最新套餐快照；同一 Dashboard 会话也可供 IDE Chat 和 IDE 执行变体目录探测使用，这些后台动作不需要前端持有 Token。
...
无论凭据来自 PKCE 还是手工兼容导入，Token 都只会加密保存在服务端，并只发送到配置项 `cursor.dashboard_base_url` 或 `cursor.chat_base_url`（默认均限定为 `https://api2.cursor.sh`）。它用于 IDE Chat、IDE 执行变体目录探测和 Dashboard 套餐探测，不会参与 Cloud Agent 创建或逻辑模型白名单同步。
...
Protobuf 响应通过 Connect 帧解码后解析 `AvailableModelsResponse`，兼容重复 `modelNames` 字段和嵌套模型消息中的 `name`；JSON codec 还读取 `models[].serverModelName`、`defaultOn`、`supportsThinking` 和 `legacySlugs`。该目录只用于 IDE 健康检查和聊天运行时变体选择；管理端白名单同步不会把这些执行变体逐项展示。已保存账号遇到过期 Token 时可自动刷新。
...
Dashboard 登录成功后会同时保存该次 PKCE 流程的 UUID 作为 `cursor_machine_id`。Cursor IDE 聊天会把此值纳入请求校验；仅保存 Token 会导致 IDE 变体目录探测成功、真实聊天却被上游误报为客户端版本不受支持。升级前已连接且缺少该值的账号，在 `auto` 模式下仍可探测 IDE `AvailableModels`，但真实聊天会暂时使用 Cloud Agent API Key；重新执行一次 Dashboard 授权后即可恢复 IDE Chat。显式设置 `cursor_transport_mode: ide_chat` 的账号缺少该值时会直接提示重新授权，不会伪造随机机器 ID。

IDE 模型目录中的公开 `name` 与聊天 RPC 要求的 `serverModelName` 可能不同。网关会通过 `legacySlugs` 把逻辑模型关联到整组执行变体，并读取 Anthropic `thinking.type` / `output_config.effort`、OpenAI Chat `reasoning_effort`、OpenAI Responses `reasoning.effort` 选择匹配的 thinking 与 effort；未显式指定时使用 Cursor 标记的 `defaultOn` 变体。旧客户端若直接发送具体 IDE 变体名仍可兼容直通。
...
- IDE 健康检查和运行时执行变体发现使用 `AvailableModels` Connect RPC；Cloud Agent 检查仍使用 `/v1/me`；白名单同步优先使用 `/v1/models`，IDE-only 账号则折叠 `AvailableModels.legacySlugs`；
