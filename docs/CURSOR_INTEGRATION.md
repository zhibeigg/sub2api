...
- 模型 ID 必须来自 `GET /v1/models`。管理后台的新建与编辑账号表单支持调用上游模型接口并把返回 ID 合并到模型白名单；省略 `model` 字段表示使用 Cursor 配置的默认模型，不应写死或猜测模型名称。
- Cursor 账号可显式启用“在 `/v1/messages` 中使用”，开启后可加入 Anthropic 分组并由兼容层处理 Messages 请求。不同上游的会话上下文与模型能力可能不同，应通过分组隔离账号，并只启用已同步且验证可用的模型。
