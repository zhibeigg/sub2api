# QQBot BotGo / SnowLuma OneBot v11 单接入方式集成指南

Sub2API `0.87.101` 支持腾讯官方 BotGo 与 SnowLuma OneBot v11 两种 QQBot 接入方式。管理员必须在后台的 **QQBot → 接入方式** 中明确二选一：BotGo 处理腾讯官方群与频道能力；OneBot 通过本机反向 WebSocket 提供普通 QQ 群消息、`group_increase` 真实进群事件以及受好友关系验证约束的私聊命令。两套加密配置会保留以便切换，但任何时刻只有已选方式的 Runtime、worker、Webhook 或反向 WebSocket 可以处理业务流量。

已有 QQBot 绑定数据、身份、首次赠送、余额流水和审计继续保存在 Sub2API PostgreSQL 中。BotGo 与 OneBot 仍使用独立机器人标识和 Redis Stream；模式切换不会删除备用配置或已入队数据，但未选 Runtime 会停止读取、发送和接收新连接。
...
```text
                         ...
                         ┌───────┴────────┐
                         ▼                ▼
              腾讯官方 BotGo          SnowLuma OneBot
              /webhooks/qq            /webhooks/qq/onebot
              Ed25519 / op=13         本机 reverse WS
                         │                │
                         └───────┬────────┘
                                 ▼
                    仅已选 Runtime 运行并消费独立 Stream
                                 ▼
                      共享命令、欢迎、绑定与审计处理器
                                 ▼
                   PostgreSQL（两套加密配置）与 Vue 管理页
```
...
1. **概览**：当前接入方式、已选 Runtime 状态、队列积压、worker、最近事件/发送时间。
2. **接入方式**：腾讯官方 BotGo 与 SnowLuma OneBot 的互斥选择；切换立即停止未选链路，并保留其脱敏配置以便日后切回。
3. **接入配置**：只显示当前选择的 BotGo 或 OneBot 表单、probe 与启用门禁，不会并列配置两条正在运行的链路。
4. **消息与欢迎**：绑定开关、首绑奖励、链接 TTL、独立帮助/欢迎文案、群/频道白名单与频道欢迎映射。
5. **绑定记录**：统计、分页、状态/场景/时间筛选和管理员解绑。
6. **诊断**：已选方式的 Runtime、配置版本、最近稳定错误码与连接 probe；BotGo 额外显示 Webhook 与域名校验 URL。
...
- `transport_mode`：`botgo` 或 `onebot`，是全局唯一的运行方式；与配置版本、审计和 Redis 失效广播一起原子更新。
- `enabled`：仅在 `transport_mode=botgo` 时决定 BotGo Runtime 是否运行。
- `app_id`
...
- `enabled`，默认 `false`；仅在全局 `transport_mode=onebot` 时决定 OneBot Runtime 是否运行。
...
- `auto_approve_friend_requests`：默认 `false`；启用后自动批准机器人收到的所有好友申请。
- `auto_approve_group_requests`：默认 `false`；启用后仅自动批准目标群已位于 `qqbot_allowed_group_ids` 白名单内的用户加群申请，不接受机器人入群邀请。
...
切换通过 `PUT /api/v1/admin/qqbot/transport` 完成，提交当前 `expected_config_version` 和 `mode`。服务端使用与 BotGo 配置相同的事务锁、审计记录和 Redis Pub/Sub 失效通知：先停止未选 Runtime 的 worker/Hub/active sink，再让已选 Runtime 按其已保存的 `enabled` 配置启动。BotGo 未选时 `/webhooks/qq` 返回不可用；OneBot 未选时 `/webhooks/qq/onebot` 不接受反向 WebSocket。

升级前的 JSON 没有 `transport_mode` 时，服务会显示“升级继承”提示：仅 OneBot 已启用时解析为 OneBot，其余情况解析为 BotGo。管理员应在“接入方式”栏目点击实际需要的选项以持久化确认；两边历史配置都不会删除。若旧部署曾同时启用两边，升级继承按 BotGo 回退，必须在切流验收时重新确认实际运行方式。
...
- `qqbot_command_cooldown_seconds`：`/help`、`/bind` 与 `/check` 的用户级冷却秒数，默认 60，允许 10–3600
- `qqbot_welcome_enabled`
- `qqbot_welcome_message`
- `qqbot_first_interaction_enabled`：兼容保留字段；运行时始终规范化为 `true`。OneBot 用户实际添加机器人好友后会发送一次开场消息，此行为不可关闭。
- `qqbot_help_message`
...
群或频道白名单为空时，对应公共场景 fail-closed。群内命令、冷却、错误和 `/check` 图片始终只回复到触发消息所在群，绝不私聊群成员。C2C 仅支持 OneBot `message_type=private` 且 `sub_type=friend` 的好友会话；BotGo C2C、群临时会话和未知私聊类型均不回应。`/check` 的权限更严格：白名单内的群/频道身份可直接使用，无需个人绑定；好友 C2C 必须存在当前机器人 AppID 下、场景与官方身份均匹配且状态为 `active` 的绑定。

`/help`、有效的 `/bind 邮箱` 与 `/check` 使用统一用户级冷却。Redis scope 同时包含机器人传输链路（BotGo/OneBot）、机器人 AppID 或 Self ID、场景、用户主体和指令，因此两条链路及不同指令互不影响；同一队列事件的重试不会重复消耗配额。冷却命中会回复准确的剩余秒数，超过一分钟时回复分钟和秒数。`/bind` 原有的每 5 分钟最多 3 次业务保护仍会在运行时继续执行。
...
- 仅当 `transport_mode=botgo` 时该入口可用；选择 OneBot 后地址校验和 dispatch 均返回不可用，避免双链路同时接收事件。
...
- 仅当 `transport_mode=onebot` 时接受连接；选择 BotGo 后该入口返回不可用，Hub、worker 和 pending action 会被清理。
...
- 后台定时读取到与当前活动连接配置完全相同的值时只更新健康快照，不会重建 Hub、worker 或中断已建立的反向 WS；只有启停、Self ID、Token、worker、队列或 Action 超时变化才会热重载。自动审批策略属于 worker 读取的业务快照，即使其保存会递增配置版本也不会切换反向 WS。
- action 使用唯一 `echo` 并发关联，限制 pending 数量、单帧大小与调用超时；断线、取消和实际配置重载会清理 pending 请求。
- 支持 `message/group`、仅限 `sub_type=friend` 的 `message/private`、`notice/group_increase`、`notice/friend_add`，以及可选的 `request/friend` 和 `request/group`（仅 `sub_type=add`）。`friend_add` 是唯一触发一次性好友开场消息的事件；普通私聊、群临时会话、未知 subtype、机器人自身事件、未知事件、机器人入群邀请和缺少关键 ID 或申请 flag 的 payload 均被忽略。请求事件使用类型、时间、Self ID、群、用户和 flag 生成稳定 SHA-256 指纹；flag 仅加密存入短期队列，绝不写入日志或审计。
- `send_group_msg`、`send_private_msg`、`get_login_info`、`set_friend_add_request` 和 `set_group_add_request` 均通过同一反向 WS 执行，不暴露额外动作端口。自动审批默认关闭，好友审批由 `auto_approve_friend_requests` 独立控制；加群审批还必须同时开启 `auto_approve_group_requests` 且目标群在 `allowed_group_ids` 白名单内。
...
白名单 QQ 群可直接发送（可选 @ 机器人）`/bind name@example.com` 发起绑定；请求会按 `group:<QQ号>` 身份隔离，所有回复仍留在该群，验证链接仅发送到该站点邮箱。邮箱命令原文会出现在群聊记录中，因此不应在群内使用不希望公开的邮箱。只有用户实际添加机器人好友并进入 OneBot 好友私聊后，才可在私聊发送指令；群内消息不会触发私聊，群临时会话也不会被处理。
...
- **QQ 群 / OneBot 好友 C2C**：群和好友 C2C 都只使用各自触发事件的回复路径。BotGo 群图片先通过腾讯文件上传接口取得 `file_info`，再以 `msg_type=7` 发送富媒体消息；同一事件重试复用固定 `msg_seq` 和已取得的 `file_info`，避免响应丢失后重复上传或重复消息；不得把内部文件路径或 HMAC 密钥放入消息体。
...
| 方法 | 路径 | 用途 |
|---|---|---|
| GET | `/api/v1/admin/qqbot/config` | 读取脱敏配置与当前 `transport_mode` |
| PUT | `/api/v1/admin/qqbot/transport` | 原子选择 `botgo` 或 `onebot`，并热切换唯一运行链路 |
| PUT | `/api/v1/admin/qqbot/config` | 原子更新 BotGo Runtime 与业务设置（仅 BotGo 被选中时可启用） |
...
OneBot 配置响应只包含 `access_token_configured`，更新请求中 `access_token` 留空表示保留现值；同时读写 `auto_approve_friend_requests` 与 `auto_approve_group_requests` 两个默认关闭的布尔开关。仅修改这两个审批策略不会重建 Hub、worker 或反向 WS；worker 在处理申请时读取当前策略。`enabled=true` 与 OneBot probe 仅在 `transport_mode=onebot` 时允许，后端强制检查当前配置对应的短期 probe 凭证；前端流程不能替代服务端门禁。
...
7. 群内 `/help`、`/bind`、`/check`、冷却和错误只在原群回复；普通群消息不会自动回复或私聊成员。
8. OneBot `notice/friend_add` 仅发送一次开场；只有 `private/friend` 中的 `/help`、`/bind` 和已绑定身份提示会在同一好友私聊回复。群临时会话、未知私聊、BotGo C2C 与 `ENTER_AIO` 不产生输出。
9. `/check` 在白名单群/频道无需绑定即可使用，OneBot 好友 C2C 只有 `active` 绑定可用；非白名单、非 active 绑定和 30 秒内重复请求均被安全拒绝。
10. `channel_check_enabled` 与 `channel_monitor_enabled` 任一关闭时 `/check` 不执行；双开后恢复。
11. BotGo 群使用腾讯 `file_info` + `msg_type=7`，频道使用公网短期 HMAC URL；OneBot 好友会话使用 URL 图片消息段；过期或篡改 URL 被拒绝。
