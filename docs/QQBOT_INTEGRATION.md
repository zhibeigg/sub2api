...
6. `/check` 在白名单群/频道无需绑定即可使用，C2C 只有 `active` 绑定可用；非白名单、非 active 绑定和 30 秒内重复请求均被安全拒绝。
7. `channel_check_enabled` 与 `channel_monitor_enabled` 任一关闭时 `/check` 不执行；双开后恢复。
8. 群/C2C 使用腾讯 `file_info` + `msg_type=7`，频道使用腾讯可访问的短期 HMAC HTTPS image URL；过期或篡改 URL 被拒绝。
9. 自定义 `QQBOT_CHANNEL_CHECK_FONT_PATH` 与镜像默认 Noto CJK 字体均可正常渲染中文；渲染、上传或发送失败时回退通用文本。
10. 邮件链接、绑定完成、余额和兑换流水。
11. 同链接并发提交、Webhook 重投和解绑后重绑不重复赠送。
12. 修改帮助文案、白名单和 worker 后热更新生效。
13. 主容器重启后 pending Stream 事件可恢复。
14. 日志和审计无密钥、完整邮箱、完整 OpenID、完整签名 URL、渠道探测正文或消息正文。
