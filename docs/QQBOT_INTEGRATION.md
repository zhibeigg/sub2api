...
Sub2API `0.86.101` 支持腾讯官方 BotGo 与 SnowLuma OneBot v11 两种 QQBot 接入方式。管理员必须在后台的 **QQBot → 接入方式** 中明确二选一：BotGo 处理腾讯官方群、C2C 与频道能力；OneBot 通过本机反向 WebSocket 提供普通 QQ 群消息与 `group_increase` 真实进群事件。两套加密配置会保留以便切换，但任何时刻只有已选方式的 Runtime、worker、Webhook 或反向 WebSocket 可以处理业务流量。
...
- `qqbot_first_interaction_enabled`：默认关闭；仅在管理员显式开启时才会在首次互动后主动发送欢迎
- `qqbot_help_message`
...
- 支持 `message/group`、`message/private`、`notice/group_increase`，以及可选的 `request/friend` 和 `request/group`（仅 `sub_type=add`）；忽略机器人自身事件、未知事件、机器人入群邀请和缺少关键 ID 或申请 flag 的 payload。请求事件使用类型、时间、Self ID、群、用户和 flag 生成稳定 SHA-256 指纹；flag 仅加密存入短期队列，绝不写入日志或审计。
