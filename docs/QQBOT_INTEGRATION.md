...
Sub2API `0.57.60` 增加供独立 `sub2api-qqbot` 服务使用的 HMAC 私有 API；`0.58.63` 补充绑定前的现有 QQ 身份检测与统一邮箱脱敏响应，并与企业微信 JSAPI 支付功能一并发布。QQBot 只接收腾讯官方事件、发送消息、代理网页与管理请求，不直接连接 Sub2API 数据库。
...
2. 部署 Sub2API `0.58.63`（包含 migration `183`、标准 MIME 邮件修复、已绑定身份检测和企业微信 JSAPI 支付）。
