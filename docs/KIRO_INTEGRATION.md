...
- **后台 UI**：账号列表「平台/类型」列正确显示 Kiro（琥珀色）；「用量窗口」列展示订阅类型、用量占比、试用、超额与上下文占比，并同步展示当日请求数、Token、账号计费（A）和用户扣费（U）；账号编辑弹窗支持「同步最新支持模型」（本地 Claude 模型集）与「同步上游支持的模型」（调用上游 `ListAvailableModels` 动态发现）。
