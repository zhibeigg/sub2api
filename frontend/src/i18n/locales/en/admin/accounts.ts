export default {
    accounts: {
      ...
      cursor: {
        ...
        dashboardCredentialsTitle: 'Official Plan Usage Credentials (Optional)',
        dashboardCredentialsHint: 'Reads the Total, First-party, and API plan meters from Cursor Dashboard. These tokens are not used for Cloud Agents forwarding. The endpoint is undocumented and gracefully falls back to local usage if it changes.',
        ...
        dashboardTokenSourceHint: 'Windows source: cursorAuth/accessToken and cursorAuth/refreshToken in %APPDATA%\\Cursor\\User\\globalStorage\\state.vscdb. Tokens are encrypted at rest and sent only to api2.cursor.sh.',
        editCredentialsTitle: 'Update Cursor Sensitive Credentials',
        editCredentialsHint: 'Cloud API Keys and Dashboard tokens are never prefilled. Keep is the default; only replacements and explicit clears are sent.',
        ...
        clearConfirm: 'Clear these Cursor credentials: {fields}? Clearing the API Key disables scheduling; clearing Dashboard tokens only disables official plan meters.',
