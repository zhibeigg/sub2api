export default {
    ...
    announcements: {
      ...
      email: {
        sendTitle: '发布邮件通知',
        sendDescription: '这是高成本外发操作。开关默认关闭，仅在公告最终状态为“展示中”且尚无邮件任务时可用。',
        sendToggle: '同时发送邮件',
        loadingCapability: '正在检查邮件能力与预计收件人数…',
        estimatedRecipients: '预计向 {count} 个具有有效主邮箱的用户排队发送',
        primaryEmailOnly: '仅使用用户当前有效的主邮箱，不展示或导出收件地址。',
        ignoresTargeting: '邮件收件范围不受公告展示条件（targeting）影响。',
        scheduledAtStarts: '已设置开始时间，邮件任务将在 starts_at 到达后开始。',
        startsImmediately: '未设置开始时间，发布后将立即进入邮件队列。',
        onceOnly: '每条公告只创建一次邮件任务；后续编辑不会自动重发。',
        featureDisabled: '公告邮件功能已关闭。请先在系统邮件设置中启用后再试。',
        smtpNotConfigured: 'SMTP 尚未配置。请先完成发信服务器配置并验证可用性。',
        activeOnly: '请先将公告最终状态设为“展示中”。草稿和归档公告不能发送邮件。',
        alreadyQueued: '此公告已有邮件任务。编辑公告不会再次发送；请在邮件状态详情中查看或重试。',
        unavailable: '当前不能为此公告发送邮件，请检查公告状态与邮件配置。',
        failedToLoadCapability: '无法检查公告邮件能力。请刷新页面；若持续失败，请检查邮件服务状态。',
        publishAndQueue: '发布并发送邮件',
        confirmTitle: '确认发布并发送邮件',
        confirmMessage: '此操作预计会为 {count} 个有效主邮箱创建一次性邮件任务，且不受展示条件限制。确认后邮件将立即或在 starts_at 到达时排队。',
        confirmSend: '发布并发送邮件',
        continueEditing: '继续编辑',
        queuedSuccess: '公告已保存，邮件通知已排队；请在邮件状态列查看实际发送进度。',
        openDetailsFor: '查看公告“{title}”的邮件状态详情',
        deliveryStatus: '投递状态',
        detailsTitle: '公告邮件状态',
        autoRefreshing: '每 5 秒自动刷新',
        refreshStatus: '刷新邮件状态',
        noStatus: '此公告尚未创建邮件任务。',
        availableAt: '可执行时间',
        startedAt: '开始时间',
        finishedAt: '完成时间',
        errorSummary: '最近错误摘要',
        retryTitle: '受控重试',
        retryHint: '仅重试失败收件项。状态不明确的收件项默认不会重试，以避免重复投递。',
        retryFailed: '重试失败项',
        retryIncludingAmbiguous: '重试失败及状态不明确项',
        retryConfirmTitle: '确认重试失败项',
        retryConfirmMessage: '将为明确失败的收件项创建新的重试任务，不会重试已发送或状态不明确的项。',
        retryAmbiguousConfirmTitle: '确认包含状态不明确项',
        retryAmbiguousConfirmMessage: '状态不明确表示服务无法确认是否已投递。包含这些项可能造成重复邮件，请仅在核实影响后继续。',
        retryQueuedSuccess: '邮件重试已排队，请继续关注实际发送状态。',
        failedToLoadStatus: '加载邮件状态失败。请稍后刷新；若持续失败，请检查邮件任务服务。',
        failedToRetry: '邮件重试未能排队。请保留当前页面并重试；若持续失败，请检查错误摘要。',
        metrics: {
          total: '总数',
          sent: '已发送',
          failed: '失败',
    ...
    promo: {
      ...
      bonusAmount: '赠送金额 ($)',
      rechargeBonusMultiplier: '首笔充值到账加成倍率',
      rechargeBonusMultiplierHint: '仅通过此优惠链接注册用户的首笔成功余额充值生效（≥1，如 1.2 表示首充多到账 20%）；留空或 1 = 无加成',
      maxUses: '最大使用次数',
      zeroUnlimited: '0 = 无限制',
      expiresAt: '过期时间',
      notes: '备注',
