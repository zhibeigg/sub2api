export default {
    settings: {
      ...
      payment: {
        ...
        wxpayH5EnabledHint: '用于微信外的移动浏览器；启用后必须配置 H5 应用信息。',
        wxpayJsapiEnabled: 'JSAPI 公众号支付',
        wxpayJsapiEnabledHint: '仅在已开通公众号支付并完成公众号 OAuth 配置时启用。',
        ...
        wxpayGuideSummary: '按实例已开通能力选路：桌面 Native、微信外移动端 H5 优先、微信内有 OpenID 时仅 JSAPI。',
        wxpayGuideNote: '请只启用微信商户平台实际开通的能力；关闭的模式会在本地阻断，不会调用对应微信接口。',
        ...
        wxpayGuideJsapiFallback: 'JSAPI 能力未启用时不启动 OAuth，并可回退扫码；已进入 JSAPI 后的授权或接口错误会结构化提示，不会盲目重试其他模式。',
