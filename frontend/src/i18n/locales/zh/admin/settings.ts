export default {
    settings: {
      ...
      site: {
        ...
        chatwoot: {
          title: '在线客服',
          description: '接入 Chatwoot Website Widget，为匿名访客和已验证的登录用户提供在线支持。',
          enabled: '已启用',
          disabled: '已关闭',
          baseUrl: 'Chatwoot Base URL',
          baseUrlPlaceholder: 'https://app.chatwoot.com',
          baseUrlHint: 'Chatwoot Cloud 或自建实例地址，末尾斜杠会在保存时自动移除。',
          websiteToken: 'Website Token',
          websiteTokenPlaceholder: 'Inbox Website Token',
          websiteTokenHint: '这是浏览器加载 Widget 所需的公开标识，不是 API Access Token。',
          identitySecret: 'Identity Validation Secret',
          identitySecretPlaceholder: '可选：用于 HMAC 身份验证',
          identitySecretConfiguredPlaceholder: '密钥已配置，留空以保留当前值。',
          identitySecretHint: '未配置时登录用户仍以匿名访客身份咨询，前端不会执行不可信的用户绑定。',
          identitySecretConfiguredHint: '服务端身份签名已配置；留空保存会保留现有密钥。',
          identityConfigured: '身份验证已配置',
          identityAnonymous: '匿名模式',
          securityTitle: '安全边界',
          securityHint: 'Identity Validation Secret 仅保存于服务端。前端只接收稳定 identifier 与 HMAC identifier_hash；生产环境建议使用 HTTPS，并确认 Base URL 指向可信的 Chatwoot 实例。',
          incompleteError: '启用 Chatwoot 前必须填写有效的 Base URL 和 Website Token。',
          invalidBaseUrlError: 'Chatwoot Base URL 必须是有效的 http(s) 地址。',
        },
        customEndpoints: {
          title: '自定义端点',
      ...
      payment: {
        ...
        balancePaymentDisabledHint: '关闭用户购买页中的余额充值入口，订阅购买不受影响。',
        subscriptionPaymentDisabled: '禁用订阅购买',
        subscriptionPaymentDisabledHint: '关闭用户购买页中的订阅套餐入口，余额充值不受影响。',
        noLimit: '留空表示不限制',
        helpImage: '帮助图片',
        ...
        validationWxpayH5AppUrl: 'H5 应用地址必须是完整的 HTTPS URL',
        validationWxpayMpAppId: '公众号 JSAPI 需要 wx 开头的 AppID；基础 App ID 为企业微信 CorpID 时，请填写公众号 AppID 或改选企业微信认证',
        validationWxpayWecomCorpId: '企业微信 JSAPI 认证要求基础 App ID 使用 ww 开头的企业 ID（CorpID）',
        validationWxpayWecomAgentId: '企业微信应用 AgentId 如填写，必须是大于 0 的整数',
        ...
        validationEasyPayCustomMethodReserved: '易支付自定义支付方式不能使用当前协议的内置支付方式',
        ...
        field_wecomAppSecret: '企业微信应用 Secret',
        field_wecomAgentId: '企业微信应用 AgentId',
        field_mchId: '商户号',
        ...
        field_mpAppIdHint: '公众号认证使用。可留空复用基础 App ID；如果基础 App ID 是 ww 开头的企业微信 CorpID，则必须填写 wx 开头的公众号 AppID。',
        field_wecomAppSecretHint: '企业微信自建应用的 Secret。该字段属于敏感凭证；编辑已有配置时留空会保留原值。',
        field_wecomAgentIdHint: '可选。填写企业微信自建应用中大于 0 的整数 AgentId；具体有效性仍以后端和企业微信接口校验为准。',
        ...
        wxpayNativeEnabledHint: '用于桌面端二维码，也可作为移动端或微信内的安全回退。',
        wxpayH5Enabled: 'H5 支付',
        wxpayH5EnabledHint: '用于微信外的移动浏览器；开关独立且默认关闭，启用后必须配置 H5 应用信息。',
        wxpayJsapiEnabled: 'JSAPI 支付',
        wxpayJsapiEnabledHint: '用于微信内的公众号或企业微信应用授权；启用后请选择对应认证类型。',
        wxpayJsapiAuthType: 'JSAPI 认证类型',
        wxpayJsapiAuthTypeHint: '公众号与企业微信使用不同的授权凭证和管理后台配置。历史配置缺少此字段时按公众号模式载入，不会根据 CorpID 自动改成企业微信。',
        wxpayJsapiAuthMp: '微信公众号',
        wxpayJsapiAuthWecom: '企业微信自建应用',
        wxpayWecomSuggested: '检测到基础 App ID 为 ww 开头的企业微信 CorpID；如需企业微信应用内支付，建议选择“企业微信自建应用”。',
        wxpayWecomSetupTitle: '企业微信 JSAPI 配置检查',
        wxpayWecomTrustedDomainHint: '在企业微信应用管理中配置可信域名，确保当前站点域名已通过企业可信域名校验。',
        wxpayWecomCallbackHint: '在企业微信应用的网页授权及 JS-SDK 配置中登记本站回调域名/地址，并确保与实际访问协议和域名一致。',
        wxpayWecomVisibilityHint: '将自建应用的可见范围覆盖需要发起支付的成员或部门，否则用户可能无法完成应用授权。',
        wxpayWecomPaymentDirectoryHint: '在微信商户平台配置支付授权目录，覆盖实际发起 JSAPI 支付的 HTTPS 页面目录。',
        wxpayConfigHint: '请先选择商户已开通的支付能力，再填写对应场景所需配置。',
        ...
        wxpayGuideSummary: '按实例已开通能力选路：桌面 Native、微信外移动端 H5 优先、微信或企业微信内取得用户标识后使用 JSAPI。',
        wxpayGuideNote: '请只启用微信商户平台实际开通的能力；H5 与 JSAPI 开关相互独立，关闭的模式会在本地阻断。',
        wxpayGuideNativeTitle: 'Native / 扫码支付',
        wxpayGuideNativeOpen: '需开通 Native 或扫码支付能力。',
        wxpayGuideNativeCall: '桌面端默认调用 Native，下发二维码内容给前台渲染。',
        wxpayGuideNativeFallback: '移动端 H5 未启用、缺少客户端 IP，或微信内 JSAPI 未启用时，可在调用微信接口前安全回退到这里。',
        wxpayGuideJsapiTitle: 'JSAPI / 公众号或企业微信',
        wxpayGuideJsapiOpen: '公众号模式需开通公众号支付并取得 OpenID；企业微信模式需配置自建应用凭证、可信域名、回调、应用可见范围与支付授权目录。',
        wxpayGuideJsapiCall: '在微信或企业微信内按所选认证类型完成授权后调用 JSAPI，直接拉起微信支付。',
        wxpayGuideJsapiFallback: 'JSAPI 未启用时不启动授权并可回退扫码；认证类型或凭证组合不明确时前端给出提示，最终校验仍以后端和微信接口为准。',
        wxpayGuideH5Title: 'H5 支付',
        wxpayGuideH5Open: '需开通 H5 支付。',
        wxpayGuideH5Call: '移动端非微信浏览器且有客户端 IP 时调用 H5 支付，跳转微信收银台。',
        wxpayGuideH5Fallback: 'H5 未启用或缺少客户端 IP 时在本地回退扫码；H5 接口返回权限或配置错误时直接提示，不再调用其他微信模式。',
        noProviders: '暂无服务商实例',
