export default {
    ...
    users: {
      ...
      columns: {
        ...
        usageAdobe: '用量 (Adobe)',
        usageCursor: '用量 (Cursor)',
        usageOpenCode: '用量 (OpenCode Go)',
        concurrency: '并发数',
        status: '状态',
        lastActive: '最后活跃时间',
        lastUsed: '最后使用时间',
      ...
      // 用户分组配置
      groupConfig: '用户分组配置',
      groupConfigHint: '访问白名单、专属分组授权与用户专属倍率彼此独立；每次打开都会读取最新配置。',
      accessMode: '标准分组访问模式',
      accessModeInherit: '继承标准分组',
      accessModeInheritHint: '保持现有语义并自动允许当前及未来新增的标准分组；切换到限制模式后才能编辑白名单。',
      accessModeRestricted: '限制模式',
      accessModeRestrictedHint: '仅允许白名单内的标准分组；专属分组还必须单独获得授权。',
      restrictedModeActive: '当前使用标准分组白名单。未来新增的标准分组不会自动允许。',
      allowAllAndInherit: '允许全部并取消限制',
      searchGroupsPlaceholder: '按名称或平台搜索',
      selectCurrentResults: '全选当前结果',
      clearCurrentResults: '清空当前结果',
      groupSelectionStatus: '已允许 {selected}/{total} 个分组，当前显示 {visible} 个',
      emptyWhitelistWarning: '白名单为空，保存后该用户将无法使用任何活跃标准分组。',
      ...
      customRateAria: '设置分组 {group} 的专属倍率',
      groupPermissionAria: '允许使用分组 {group}',
      exclusiveGrant: '专属授权',
      exclusiveGrantAria: '授权用户使用专属分组 {group}',
      disabledGroupRatePreserved: '当前不可编辑，但已保存倍率会保留；恢复访问或专属授权后继续生效。',
      invalidCustomRate: '有 {count} 个专属倍率无效，请输入大于 0 的数字或清空。',
      confirmGroupConfigTitle: '确认保存分组配置',
      confirmGroupConfigMessage: '将以“{mode}”保存，允许 {selected}/{total} 个当前分组。确认继续吗？',
      groupConfigLoadFailed: '加载用户分组配置失败',
      groupConfigSaveFailed: '保存用户分组配置失败',
