export default {
  qqbot: {
    ...
    tabs: { overview: '概览', config: 'BotGo 配置', onebot: 'SnowLuma / OneBot', messages: '消息与欢迎', bindings: '绑定记录', diagnostics: '诊断' },
    ...
    onebot: {
      title: 'SnowLuma / OneBot v11', description: '通过本机反向 WebSocket 并行接入普通 QQ 群消息与进群事件，不替换腾讯官方 BotGo 链路。', connected: '反向 WS 已连接', disconnected: '反向 WS 未连接', selfId: '机器人 QQ / Self ID', selfIdHint: '必须与 SnowLuma 当前登录账号一致，仅保存数字 ID。', accessToken: 'Bearer Access Token', tokenHint: '至少 32 个字符；加密保存且 API 只返回是否已配置。', reverseWsUrl: 'SnowLuma 反向 WebSocket URL', reverseWsHint: '请在 SnowLuma 中使用该本机地址，配置相同 Token，并保持 3010/3011 仅监听 loopback。', actionTimeout: 'Action 超时（毫秒）', enableRuntime: '启用 OneBot 事件处理', runtimeState: '运行状态', workers: 'Worker', pendingActions: '待响应 Action', lastConnection: '最近连接活动'
    },
    messages: {
      title: '消息与欢迎', description: '配置帮助回复、账户绑定、首绑奖励以及群和频道的欢迎范围。白名单群可直接发送 /bind 邮箱；邮箱原文会出现在群聊记录中。', bindingEnabled: '允许账户绑定', bindingEnabledHint: '关闭后，待处理链接无法完成绑定；白名单群内可直接发送 /bind 加邮箱。', welcomeEnabled: '新成员欢迎', welcomeEnabledHint: '在支持的群或频道成员加入事件中发送欢迎。', firstInteraction: '首次互动欢迎', firstInteractionHint: '用户首次提及或私聊机器人时发送欢迎。', channelCheckEnabled: '允许 /check 渠道状态图', channelCheckEnabledHint: '开启后，/check 可生成渠道状态图；同时要求公网 HTTPS 根域和服务端显式配置共享 TOTP 加密密钥。', firstBindBonus: '首次绑定奖励', linkTtl: '绑定链接有效期（分钟）', welcomeMessage: '入群欢迎文案', welcomeMessageHint: "支持 {'{'}site{'}'}、{'{'}user{'}'}、{'{'}bind_command{'}'} 纯文本占位；关闭绑定或 /check 后，对应指令行不会发送。最多 4000 字符，当前 {count} 字符。", helpMessage: '帮助文案', helpMessageHint: '最多 4000 字符，当前 {count} 字符。', allowlistTitle: '群与频道范围', allowlistHint: '群或频道白名单为空时将拒绝对应来源（fail-closed），并非不限制；每项按腾讯提供的原始 ID 精确匹配。', allowedGroups: '允许的群 ID', allowedGuilds: '允许的频道 ID', onePerLine: '每行一个 ID', welcomeChannels: '频道欢迎子频道映射', welcomeChannelsHint: '每行使用“频道 ID = 子频道 ID”。'
    },
    bindings: {
    ...
    notices: { saved: 'QQBot 配置已保存', probeSucceeded: '腾讯连接测试通过', oneBotSaved: 'OneBot 配置已保存', oneBotProbeSucceeded: 'OneBot 反向 WebSocket 探测通过', unbound: '绑定已解除' },
