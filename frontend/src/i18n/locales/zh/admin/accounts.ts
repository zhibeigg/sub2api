export default {
    accounts: {
      ...
      cursor: {
        ...
        dashboardCredentialsTitle: '官方套餐用量凭据（可选）',
        dashboardCredentialsHint: '用于读取 Cursor Dashboard 的 Total、First-party 与 API 套餐进度，不参与 Cloud Agents 转发。接口未公开稳定承诺，失效时会自动降级为本地用量。',
        ...
        dashboardTokenSourceHint: 'Windows 来源：%APPDATA%\\Cursor\\User\\globalStorage\\state.vscdb 中的 cursorAuth/accessToken 与 cursorAuth/refreshToken。Token 仅加密保存在服务端，并只发送到 api2.cursor.sh。',
        editCredentialsTitle: '更新 Cursor 敏感凭据',
        editCredentialsHint: 'Cloud API Key 与 Dashboard Token 明文均不会回填。默认保留，仅替换或明确清除时发送变更。',
        configured: '已配置',
        notConfigured: '未配置',
        keep: '保留',
        replace: '替换',
        ...
        clearConfirm: '确认清除以下 Cursor 敏感凭据：{fields}？清除 API Key 会导致账号无法调度；清除 Dashboard Token 只会停用官方套餐进度。',
