  ...
  <AppLayout>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-group="{ row }">
            <div v-if="getSubscriptionGroups(row).length" class="flex max-w-sm flex-wrap gap-1.5">
              <GroupBadge
                v-for="group in getSubscriptionGroups(row)"
                :key="group.id"
                :name="group.name"
          ...
          <template #cell-usage="{ row }">
            <div class="min-w-[280px] space-y-2">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <div class="h-1.5 flex-1 rounded-full bg-gray-200 dark:bg-dark-600">
                    <div
                      ...
                      :class="getProgressClass(row.daily_usage_usd, getQuotaLimit(row, 'daily'))"
                      :style="{
                        width: getProgressWidth(row.daily_usage_usd, getQuotaLimit(row, 'daily'))
                      }"
                    ></div>
                  </div>
              ...
              <!-- Weekly Usage -->
              <div v-if="getQuotaLimit(row, 'weekly')" class="usage-row">
                <div class="flex items-center gap-2">
                  <span class="usage-label">{{ t('admin.subscriptions.weekly') }}</span>
                  <div class="h-1.5 flex-1 rounded-full bg-gray-200 dark:bg-dark-600">
                    <div
                      class="h-1.5 rounded-full transition-all"
                      :class="getProgressClass(row.weekly_usage_usd, getQuotaLimit(row, 'weekly'))"
                      :style="{
                        width: getProgressWidth(row.weekly_usage_usd, getQuotaLimit(row, 'weekly'))
                      }"
                    ></div>
                  </div>
                  <span class="usage-amount">
              ...
              <!-- Monthly Usage -->
              <div v-if="getQuotaLimit(row, 'monthly')" class="usage-row">
                <div class="flex items-center gap-2">
                  <span class="usage-label">{{ t('admin.subscriptions.monthly') }}</span>
                  <div class="h-1.5 flex-1 rounded-full bg-gray-200 dark:bg-dark-600">
                    <div
                      class="h-1.5 rounded-full transition-all"
                      :class="getProgressClass(row.monthly_usage_usd, getQuotaLimit(row, 'monthly'))"
                      :style="{
                        width: getProgressWidth(row.monthly_usage_usd, getQuotaLimit(row, 'monthly'))
                      }"
                    ></div>
                  </div>
                  <span class="usage-amount">
              ...
              <div
                v-if="
                  !getQuotaLimit(row, 'daily') &&
                  !getQuotaLimit(row, 'weekly') &&
                  !getQuotaLimit(row, 'monthly')
                "
    ...
    >
      ...
      >
        ...
        <div v-if="selectedAssignPlan" class="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-dark-600 dark:bg-dark-800">
          <div class="flex flex-wrap gap-1.5">
            <GroupBadge
              v-for="group in getPlanGroups(selectedAssignPlan)"
              :key="group.id"
              :name="group.name"
...
const planOptions = computed(() =>
  plans.value.map(plan => ({
      value: plan.id,
      label: `${plan.name} · $${plan.price.toFixed(2)} · ${plan.validity_days} ${t('payment.admin.' + (plan.validity_unit || 'days'))}`,
    }))
)

const selectedAssignPlan = computed(() =>
  plans.value.find(plan => plan.id === assignForm.plan_id) || null,
)

watch(() => assignForm.plan_id, () => {
  if (selectedAssignPlan.value) {
    assignForm.validity_days = selectedAssignPlan.value.validity_days
  }
})
...
// Helper functions
function asGroupPlatform(platform: string): Group['platform'] {
  return platform as Group['platform']
}
...
function getPlanGroups(plan: SubscriptionPlan): SubscriptionPlan['groups'] {
  ...
  const groupIds = plan.group_ids?.length ? plan.group_ids : [plan.group_id]
  return groupIds
...
function formatQuotaLimit(limit: number | null | undefined): string {
  return limit == null ? t('admin.subscriptions.unlimited') : `$${limit.toFixed(2)}`
}
