...
- **内蔵 QQBot デュアルトランスポート運用機能** - Tencent 公式 BotGo Webhook/メッセージ Runtime を維持しながら、通常の QQ グループメッセージと実際の `group_increase` 参加歓迎に対応する独立した SnowLuma OneBot v11 逆 WebSocket を並行実行できます。両トランスポートはコマンドとバインドルールを共有しつつ、Redis Stream、暗号化認証情報、ホット有効化/無効化、ヘルス診断、歓迎許可リストを分離します。OneBot は直結したループバックまたはプライベートネットワークの対向だけを受け入れ、Bearer Token、期待する `X-Self-ID`、単一アクティブ接続、action/echo タイムアウトを必須とし、公開アクションポートは不要です。`/check` のチャネル可用性 PNG、ワンタイムメール認証、トランスポート単位の ID バインド、初回バインド残高付与、交換台帳、監査履歴は引き続きトランザクションで保護されます（[統合・API・セキュリティ・移行・ロールバックガイド](docs/QQBOT_INTEGRATION.md)）
