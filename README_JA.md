...
- **プールモードアカウント容量予測アラート** - グループ単位で、最終的に選択された `pool_mode` アカウントの予測を有効化できます。最終グループの直近 50 件の成功課金からアカウント、API Key、ウォレットの残りリクエスト容量を推定し、50 未満の場合は管理者メールと現在の QQBot C2C バインドへ永続的に通知します。評価と再試行はゲートウェイ課金をブロックしません（[設計・設定・管理 API ガイド](docs/POOL_CAPACITY_ALERTS.md)）
