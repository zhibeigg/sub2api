...
- **Cursor ドキュメントチャット統合** - アカウント作成前の2段階認証情報チェック、手動 `_vcrcs` Cookie、Anthropic Messages / OpenAI Chat Completions / Responses 互換、Redis `previous_response_id`、ローカル使用量課金に対応。OAuth や公式購読クレジットを提供するものではありません（[統合ガイド](docs/CURSOR_INTEGRATION.md)）
