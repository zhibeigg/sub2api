...
- **マルチアカウント管理** - 複数の上流アカウントタイプ（OAuth、APIキー、サービスアカウント）をサポートし、Anthropic、OpenAI、Gemini、Antigravity、Grok、Kiro（AWS CodeWhisperer、Claude モデルを提供）、Adobe Firefly、Cursor IDE Chat / Cloud Agent を統合
- **Cursor IDE Chat + Cloud Agent** - 通常の Anthropic Messages、OpenAI Chat Completions、OpenAI Responses を、低遅延の HTTP/2 Connect-Protobuf IDE ストリームへ転送し、テキスト、reasoning、ネイティブ tool call をリアルタイム配信します。公式 Cloud Agents API は明示的なタスクモードとして維持し、Dashboard Token はサーバー側で暗号化・自動更新します（[統合ガイド](docs/CURSOR_INTEGRATION.md)）
...
## Cursor IDE Chat / Cloud Agent サポート

Cursor アカウントは `auto`、`ide_chat`、`cloud_agent` の 3 モードを利用できます。通常の `/v1/messages`、`/v1/chat/completions`、`/v1/responses` は、IDE モードで Cursor IDE の HTTP/2 `StreamUnifiedChatWithTools` に転送されます。公式 `https://api.cursor.com/v1/agents` API は、独立した明示的タスクモードとして維持されます。

- IDE Chat は暗号化された Dashboard Access/Refresh Token、専用の HTTP/2-only 接続プール、Connect-Protobuf フレームを使用し、テキスト、reasoning、MCP tool call をリアルタイム SSE へ変換します。ツールなしは Ask、ツールありは Agent モードです。
- `auto` は Dashboard Access Token があれば IDE Chat を優先し、なければ Cloud Agent を使用します。IDE リクエスト失敗後に Cloud Agent へ暗黙再送しません。
...
- IDE モデル検出は Connect-Protobuf `AiService/AvailableModels` RPC、Cloud Agent モデル検出は `GET /v1/models`、API Key 検証は `GET /v1/me` を使用します。
