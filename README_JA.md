...
- **内蔵決済システム** - 同じ `easypay` プロバイダーをバージョン指定して EasyPay V1 MD5 と Rainbow EasyPay 2.0 RSA-SHA256 に対応し、Alipay、WeChat Pay、上流チャネルで有効な QQ Pay を利用できます。Alipay/WeChat Pay の直接接続や Stripe と併用でき、フロントエンドの 3 つの決済方法は管理者がそれぞれ決済ソースを選択します。初回チャージ特典、注文帰属、純チャージ集計、絞り込み CSV 出力も引き続き内蔵されています（[設定ガイド](docs/PAYMENT.md)）
