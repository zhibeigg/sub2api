...
- **マルチアカウント管理** - 複数の上流アカウントタイプ（OAuth、APIキー、Cookie）をサポートし、Anthropic、OpenAI、Gemini、Antigravity、Grok、Kiro（AWS CodeWhisperer、Claude モデルを提供）、Adobe Firefly、Cursor ドキュメントチャットを統合
- **Cursor ドキュメントチャット統合** - 手動 `_vcrcs` Cookie、Anthropic Messages / OpenAI Chat Completions / Responses 互換、Redis `previous_response_id`、ローカル使用量課金に対応。OAuth や公式購読クレジットを提供するものではありません（[統合ガイド](docs/CURSOR_INTEGRATION.md)）
...
- **Adobe Firefly ネイティブ統合** - IMS 認証情報の安全な管理と自動更新、profile/credits 表示、OpenAI Images 互換の画像生成・編集、Redis ベースの非同期動画タスク、成功ポーリング時の冪等メディア課金に対応（[統合ガイド](docs/ADOBE_INTEGRATION.md)）
...
- **内蔵決済システム** - EasyPay、Alipay、WeChat Pay、Stripe に対応。プロモコードの残高ボーナスは初回のチャージ成功時のみ適用でき、別途決済サービスのデプロイは不要（[設定ガイド](docs/PAYMENT.md)）
...
- Adobe アカウントは `oauth` タイプを再利用しますが、ブラウザ OAuth の第 2 ステップを使わない単一ステップの認証情報入力です。
