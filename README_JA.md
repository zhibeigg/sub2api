...
2 種類の購読プランに対応します。1 つの購読グループではグループ固有の上限を使用し、1 つ以上の標準（残高）グループではプランの日・週・月上限を共有します。管理者割り当てと購入表示の個別制御にも対応します。API キーページでは複数エンドポイントのブラウザ側レイテンシ測定、3 秒ごとの自動更新、ノード詳細、組み込みの `www.pokeapi.top` バックアップ入口を利用でき、決済有効時はヘッダー残高から直接チャージ画面へ移動できます。モデル広場ではトークン、リクエスト単位、画像単位、動画単位の課金を区別し、バックエンドで算出した有効倍率と設定済みの画像・動画価格帯を表示します。デスクトップではフィルターを 2 列で表示し、モデル結果とは独立してスクロールできます。モデルカードには内部アカウントのプラットフォームではなくモデルブランドが表示され、クリックするとプラットフォーム別の実エンドポイント、チャネル、グループ、課金方式、現在倍率、倍率適用後価格を確認できるレスポンシブドロワーが開きます。
...
- **マルチアカウント管理** - 複数の上流アカウントタイプ（OAuth、APIキー、サービスアカウント）をサポートし、Anthropic、OpenAI、Gemini、Antigravity、Grok、Kiro（AWS CodeWhisperer、Claude モデルを提供）、Adobe Firefly、Cursor IDE Chat / Cloud Agent、および独立した `platform=opencode` の OpenCode Go API Key プラットフォームを統合。Messages / Chat Completions / Responses の 3 入口に対応します（[統合ガイド](docs/OPENCODE_GO_INTEGRATION.md)）
...
- **内蔵 QQBot 運用機能** - Tencent 公式 BotGo Webhook とメッセージ Runtime を Sub2API 内で実行し、暗号化された認証情報、ホット有効化/無効化、ヘルス診断、ウェルカムポリシー、バインド記録、専用管理画面を提供します。メール検証、OpenID の原子的バインド、初回ボーナス、台帳、監査履歴はトランザクションと冪等性で保護されます（[統合・移行・ロールバックガイド](docs/QQBOT_INTEGRATION.md)）
...
- `auto` が Cloud Agent へフォールバックできるのは、下流レスポンスが未コミットで、設定された安全に再送可能なエラーに一致した場合だけです。Cloud Agent は通常チャット経路ではなく、引き続き明示的タスクモードです。
