...
- **マルチアカウント管理** - 複数の上流アカウントタイプ（OAuth、APIキー、サービスアカウント）をサポートし、Anthropic、OpenAI、Gemini、Antigravity、Grok、Kiro（AWS CodeWhisperer、Claude モデルを提供）、Adobe Firefly、Cursor IDE Chat / Cloud Agent、および独立した `platform=opencode` の OpenCode Go API Key プラットフォームを統合。Messages / Chat Completions / Responses の 3 入口に対応します。グループは複数の `endpoint_protocols` を宣言し、`quota_platform` で既存のクォータ・価格帰属を維持します。検証済みのクロスプロバイダー関連は、既定で無効な `gateway.cross_provider_compatibility_enabled` で明示的に有効化できます。`gateway.group_endpoint_routing_enabled=true` ではスケジューラのスナップショットと DB フォールバックが入口プロトコル単位になり、緊急時に無効化すると従来のプロバイダーバケットへ戻ります（[統合ガイド](docs/OPENCODE_GO_INTEGRATION.md)）。
...
- **グループ容量表示とアラート** - グループ管理は現在ページの USD 残高と設定可能な残り容量を表示します。従来の履歴リクエストコストから残りリクエスト数を推定する方式に加え、管理者が設定した画像 1 枚あたりの固定アカウント容量コストから残り画像数を推定でき、完全値、既知の下限、無制限、データ不足を区別します。表示アルゴリズムはアラート指標から独立しており、50 サンプルの `predicted_requests` とグループ単位の `remaining_balance_usd` アラートの意味は変わりません（[設計・設定・管理 API ガイド](docs/POOL_CAPACITY_ALERTS.md)）
...
- **内蔵 QQBot デュアルトランスポート運用機能** - Tencent 公式 BotGo Webhook/メッセージ Runtime を維持しながら、通常の QQ グループメッセージと実際の `group_increase` 参加歓迎に対応する独立した SnowLuma OneBot v11 逆 WebSocket を並行実行できます。両トランスポートはコマンドとバインドルールを共有しつつ、Redis Stream、暗号化認証情報、ホット有効化/無効化、ヘルス診断、歓迎許可リストを分離します。変更のない OneBot 設定更新では既存の逆 WebSocket を維持し、実際の実行時設定変更時だけ切り替えます。管理者は友だち申請の自動承認と許可リスト済みグループへのユーザー参加申請の自動承認を個別に有効化できます。どちらも既定では無効で、ボットへのグループ招待は自動承認されず、申請 flag は暗号化キュー内にのみ保持されます。許可リスト済みグループでは `/bind name@example.com` を直接実行できます（メールアドレス本文はグループ履歴に表示されます）。グループ起点の出力は常に同じグループ内に留まり、認証リンクはそのメールアドレスにのみ送信されます。OneBot は実際の `friend_add` 後に新しい友だちへ一度だけ開始メッセージを送り、その後は `private/friend` の友だち会話内のコマンドだけを処理します。一時的または未知のプライベート会話は無視されます。OneBot は直結したループバックまたはプライベートネットワークの対向だけを受け入れ、Bearer Token、期待する `X-Self-ID`、単一アクティブ接続、action/echo タイムアウトを必須とし、公開アクションポートは不要です。`/check` のチャネル可用性 PNG、ワンタイムメール認証、トランスポート単位の ID バインド、初回バインド残高付与、交換台帳、監査履歴は引き続きトランザクションで保護されます（[統合・API・セキュリティ・移行・ロールバックガイド](docs/QQBOT_INTEGRATION.md)）
...
- **2 種類の購読プラン** - `subscription` プランは 1 つの購読グループだけを関連付け、そのグループ固有の上限を使用します。`standard_quota` プランは 1 つ以上の標準（残高）グループを関連付け、日・週・月の共有上限と任意の同時実行数上限を設定できます。GPT/OpenAI のネイティブ購読プランは `GPT 稳定分组 无限制` 標準グループへ移行され、プランとユーザーのクォータスナップショット、使用量、有効期限を保持します。複数の有効な旧 GPT 購読を持つユーザーは、各期間の残りクォータを 1 つの対象購読へ合算し、最も遅い有効期限を保持します。そのグループの現在の倍率で課金されます。クォータと同時実行数は購入または割り当て時に購読インスタンスへスナップショットされ、その後のプラン編集は既存購読へ遡及しません。有効なプランは対象の標準グループで残高課金より優先され、期限切れ後は公開グループが残高課金に戻ります
...
モデルゲートウェイのエラーは安全な日本語・英語・中国語表示に対応します。クライアントは `Accept-Language` で言語を選択でき、未指定または未対応の場合は `gateway.model_error_default_locale`（`en` または `zh`、既定値 `zh`）へフォールバックします。Vue ルートの render/setup/ライフサイクル例外は再試行とホーム復帰を備えた回復可能な境界で隔離され、起動時例外は内部詳細を含まない安全な DOM フォールバックを表示します。クライアント向けエラーには `[PokeAPI]` と安定した `X-PokeAPI-Error-Code`、`X-PokeAPI-Request-ID`、`Content-Language` ヘッダーが付き、上流の生エラー本文や内部例外は返しません。
