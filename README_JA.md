...
- **Cursor Cloud Agents API（Beta）** - `https://api.cursor.com`、ユーザーまたは agent-scoped サービスアカウント API Key、`/v1/me` の認証確認と手動更新チェック、`/v1/models` のモデル検出、ローカルの日次/週次/総使用量ウィンドウ、キャッシュ書き込み/読み取り Token の課金を使用し、リポジトリ連携 Agent と一時的なリポジトリなし Agent に対応します。Cursor 側のプラン/従量課金と Sub2API のローカル課金は明確に分離します（[統合ガイド](docs/CURSOR_INTEGRATION.md)）
