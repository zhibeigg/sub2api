...
2 種類の購読プランに対応します。1 つの購読グループではグループ固有の上限を使用し、1 つ以上の標準（残高）グループではプランの日・週・月上限を共有します。管理者割り当てと購入表示の個別制御にも対応します。API キーページでは複数エンドポイントのブラウザ側レイテンシ測定、3 秒ごとの自動更新、ノード詳細、組み込みの `www.pokeapi.top` バックアップ入口を利用でき、決済有効時はヘッダー残高から直接チャージ画面へ移動できます。モデル広場ではトークン、リクエスト単位、画像単位の課金を区別し、利用可能なグループの有効な画像倍率と設定済みの 1K/2K/4K 画像単価を表示します。デスクトップではフィルターを 2 列で表示し、モデル結果とは独立してスクロールできます。
