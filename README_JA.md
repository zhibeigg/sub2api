...
- **Cursor Cloud Agents API（Beta）** - `https://api.cursor.com`、ユーザーまたは agent-scoped サービスアカウント API Key、`/v1/me` の認証確認、`/v1/models` のモデル検出を使用し、リポジトリ連携 Agent と一時的なリポジトリなし Agent に対応します。Cursor 側のプラン/従量課金と Sub2API のローカル課金は明確に分離します（[統合ガイド](docs/CURSOR_INTEGRATION.md)）
...
- `GET /v1/models` で利用可能なモデル ID とパラメータを取得します。Cursor の既定モデルを使う場合は `model` を省略します。
...
- Cursor のプラン使用量、モデル使用量、Cloud Agent 実行、オンデマンド超過は Cursor 側の請求です。Sub2API はローカルで明示設定した価格だけを課金し、ローカル使用量を Cursor 公式 credits として表示しません。
