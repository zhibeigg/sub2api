...
- **グループ容量アラート** - 各グループは従来の 50 サンプルによる `predicted_requests` モードを維持するか、`remaining_balance_usd` をグループ予測残高として利用できます。これはグループ内のプールモードアカウントの権威ある USD 残高合計と、通常アカウントの推定 USD 残高合計です。グループ内の任意のアカウントで課金が成功すると USD 再計算を開始でき、しきい値を厳密に下回る場合だけ管理者へ通知します。`unknown / stale`、単位不一致、その他の不完全なデータは 0 と見なさず、グループのアラート状態も変更しません（[設計・設定・管理 API ガイド](docs/POOL_CAPACITY_ALERTS.md)）
