...
2 種類の購読プランに対応します。1 つの購読グループではグループ固有の上限を使用し、1 つ以上の標準（残高）グループではプランの日・週・月上限を共有します。管理者割り当てと購入表示の個別制御にも対応します。API キーページでは複数エンドポイントのブラウザ側レイテンシ測定、3 秒ごとの自動更新、ノード詳細、組み込みの `www.pokeapi.top` バックアップ入口を利用でき、決済有効時はヘッダー残高から直接チャージ画面へ移動できます。モデル広場ではトークン、リクエスト単位、画像単位の課金を区別し、利用可能なグループの有効な画像倍率と設定済みの 1K/2K/4K 画像単価を表示します。
...
- **内蔵決済システム** - 同じ `easypay` プロバイダーをバージョン指定して EasyPay V1 MD5 と Rainbow EasyPay 2.0 RSA-SHA256 に対応し、Alipay、WeChat Pay、上流チャネルで有効な QQ Pay を利用できます。Alipay/WeChat Pay の直接接続や Stripe と併用でき、WeChat Pay 直接接続では Native/H5/JSAPI の能力を個別に宣言し、クライアント環境に応じて安全に選択し、無効なモードをローカルで遮断します。フロントエンドの 3 つの決済方法は管理者がそれぞれ決済ソースを選択し、初回チャージ特典、注文帰属、純チャージ集計、絞り込み CSV 出力も引き続き内蔵されています（[設定ガイド](docs/PAYMENT.md)）
