...
- **Cursor ドキュメントチャット統合** - 任意の Chrome/Edge ログインヘルパーが、ユーザー操作後に Cursor を開いて `_vcrcs` だけを取り込みます。手動 Cookie 入力も保持し、アカウント作成前の2段階認証情報チェック、Anthropic Messages / OpenAI Chat Completions / Responses 互換、Redis `previous_response_id`、ローカル使用量課金に対応します。OAuth や公式購読クレジットを提供するものではありません（[統合ガイド](docs/CURSOR_INTEGRATION.md)）
