...
- **プールモードアカウント容量アラート** - 各グループは、最終的に選択された `pool_mode` アカウントに対して 1 つの指標を有効化できます。カスタム整数しきい値付きの予測残りリクエスト数、またはカスタム USD しきい値付きの権威ある上流残高です。リクエストモードは直近 50 件の成功課金サンプルを使用し、USD モードは最初の適格な課金から評価し、リクエスト数や平均コストから金額を捏造しません。値がしきい値を厳密に下回る場合のみ管理者メールと現在の QQBot C2C バインドへ永続通知し、`stale / unknown`、非権威、単位不一致のスナップショットは episode を変更しません（[設計・設定・管理 API ガイド](docs/POOL_CAPACITY_ALERTS.md)）
