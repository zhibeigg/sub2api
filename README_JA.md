...
2 種類の購読プランに対応します。1 つの購読グループではグループ固有の上限を使用し、1 つ以上の標準（残高）グループではプランの日・週・月上限を共有します。管理者割り当てと購入表示の個別制御にも対応します。API キーページでは複数エンドポイントのブラウザ側レイテンシ測定、ノード詳細、組み込みの `www.pokeapi.top` バックアップ入口を利用でき、決済有効時はヘッダー残高から直接チャージ画面へ移動できます。
...
- **マルチアカウント管理** - 複数の上流アカウントタイプ（OAuth、APIキー、サービスアカウント）をサポートし、Anthropic、OpenAI、Gemini、Antigravity、Grok、Kiro（AWS CodeWhisperer、Claude モデルを提供）、Adobe Firefly、Cursor 公式 Cloud Agents API を統合
- **Cursor Cloud Agents API（Beta）** - `https://api.cursor.com` の Agent に加え、サーバーのワンクリック PKCE で独立 Dashboard セッションを作成し、Spending メーター、暗号化 Token 管理、期限 1272 時間前の更新、定期プローブ、`stale` / `reauth_required` フォールバックを提供します。Dashboard Token はフロントエンドへ返さず、Cursor 側のプラン/従量課金と Sub2API のローカル課金を分離します（[統合ガイド](docs/CURSOR_INTEGRATION.md)）
...
- **優先順位付きマルチグループ API Key** - 1つのキーに複数のモデルグループを割り当て、検索、ドラッグによる優先順位変更、実効倍率表示に対応します。`X-Sub2API-Group-ID` でバインド済みグループを明示選択でき、Playground は全バインドからルーティング可能なモデルを検出して Chat、Compare、Image、Video を正しいグループへ送信します（[API ガイド](docs/API_KEY_MULTI_GROUP.md)）
...
- **内蔵決済システム** - EasyPay、Alipay、WeChat Pay、Stripe に対応。プロモコードの残高ボーナスは初回のチャージ成功時のみ適用できます。管理画面の注文管理では、登録プロモコード別の帰属、期間フィルター、サーバー集計の純チャージ額、現在の絞り込み条件に基づく CSV 出力を利用でき、別途決済サービスのデプロイは不要です（[設定ガイド](docs/PAYMENT.md)）
- **2 種類の購読プラン** - `subscription` プランは 1 つの購読グループだけを関連付け、そのグループ固有の上限を使用します。`standard_quota` プランは 1 つ以上の標準（残高）グループを関連付け、プラン内で日・週・月の共有上限を設定します。有効なプランは対象の標準グループで残高課金より優先され、期限切れ後は公開グループが残高課金に戻ります
...
## Cursor Cloud Agents 公式 API サポート

Sub2API の Cursor 統合ドキュメントは、`https://api.cursor.com` の公式 Cloud Agents API Beta を対象とします。認証には Cursor Dashboard で発行したユーザー API Key、または agent-scoped のチームサービスアカウント API Key を使用し、ブラウザ Cookie や旧ログインヘルパー拡張は使用しません。

- `GET /v1/me` で Key を検証します。ユーザー Key は所有者情報を返す場合がありますが、サービスアカウント/チーム Key は個人所有者フィールドを省略します。
...
- Cursor アカウントは、サーバーのワンクリック PKCE 認証で独立した Dashboard セッションを作成できます。管理画面は `POST /api/v1/admin/cursor/dashboard-auth/start` と `/poll` で認証を開始・ポーリングし、Dashboard Token はサーバー側で暗号化され、フロントエンドには返されません。バックグラウンド処理は JWT 期限の 1272 時間前から更新し、非公開の `api2.cursor.sh` 使用量 RPC を定期的にプローブします。このセッションは手動インポートした Desktop Token を再利用しないため、Desktop の通常終了、再起動、Token ローテーションは通常サーバーへ影響しません。ただし Cursor がアカウント全体またはサーバー側で失効処理を行う場合があり、その際 Sub2API は `reauth_required` を表示します。`state.vscdb` からの手動 Token 取り込みは高度な互換手段のみです。プローブ失敗時は `stale` スナップショットを保持し、失効・回復不能な認証情報は `reauth_required` に降格しますが、ローカル使用量や Cloud Agents API Key の状態には影響しません。
