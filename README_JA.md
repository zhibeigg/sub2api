...
- **プールモードアカウント容量予測アラート** - グループ単位で、最終的に選択された `pool_mode` アカウントの予測を有効化できます。検証済み上流残高を最終グループ直近 50 件の成功課金の平均コストで換算し、ローカルアカウントクォータは安全上限としてのみ使用します。API Key クォータとウォレットは課金後状態を維持し、50 未満の場合は管理者メールと現在の QQBot C2C バインドへ永続的に通知します。`stale` または未検証の上流データはアラート episode を変更しません（[設計・設定・管理 API ガイド](docs/POOL_CAPACITY_ALERTS.md)）
