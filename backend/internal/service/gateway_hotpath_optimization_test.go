...
func TestGetUserGroupRateMultiplier_CacheHitAndNilRepo(t *testing.T) {
	...
	key := "101:202"
	svc.userGroupRateCache.Set(key, userGroupRateCacheEntry{multiplier: 2.3, hasOverride: true}, time.Minute)
	...
	svc2.userGroupRateCache.Set(key, userGroupRateCacheEntry{multiplier: 1.9, hasOverride: true}, time.Minute)
	...
	svc2.userGroupRateCache.Set(key, userGroupRateCacheEntry{}, time.Minute)
	require.Equal(t, 1.5, svc2.getUserGroupRateMultiplier(context.Background(), 101, 202, 1.5))
