...
const (
	...
	NotificationEmailEventPoolCapacityLow             = "account.pool_capacity_low"
...
type NotificationEmailBatchRenderer struct {
	Event     string
	SiteName  string
	Templates map[string]NotificationEmailTemplate
}

func (s *NotificationEmailService) PrepareBatchRenderer(ctx context.Context, event string) (*NotificationEmailBatchRenderer, error) {
	_, normalizedEvent, err := s.eventInfo(event)
	if err != nil {
		return nil, err
	}
	r := &NotificationEmailBatchRenderer{Event: normalizedEvent, SiteName: s.siteName(ctx), Templates: make(map[string]NotificationEmailTemplate, len(notificationEmailLocales))}
	for _, locale := range notificationEmailLocales {
		tmpl, err := s.GetTemplate(ctx, normalizedEvent, locale)
		if err != nil {
			return nil, err
		}
		r.Templates[locale] = tmpl
	}
	return r, nil
}

func (r *NotificationEmailBatchRenderer) Render(locale, recipientEmail, recipientName string, variables map[string]string) (NotificationEmailPreview, error) {
	if r == nil {
		return NotificationEmailPreview{}, errors.New("nil notification email batch renderer")
	}
	locale = normalizeNotificationLocale(locale)
	tmpl, ok := r.Templates[locale]
	if !ok {
		tmpl = r.Templates[notificationEmailDefaultLocale]
	}
	values := make(map[string]string, len(variables)+3)
	for key, value := range variables {
		values[key] = value
	}
	values["site_name"] = r.SiteName
	values["recipient_email"] = strings.TrimSpace(recipientEmail)
	values["recipient_name"] = strings.TrimSpace(recipientName)
	return renderNotificationEmail(r.Event, tmpl.Subject, tmpl.HTML, values, nil)
}
...
func renderNotificationEmail(event, subject, htmlBody string, variables map[string]string, rawHTMLVariables map[string]string) (NotificationEmailPreview, error) {
	...
	renderedSubject = sanitizeEmailHeader(renderedSubject)
	if len([]rune(renderedSubject)) > notificationEmailMaxSubjectLength {
		return NotificationEmailPreview{}, fmt.Errorf("rendered email subject cannot exceed %d characters", notificationEmailMaxSubjectLength)
	}
	if len([]byte(renderedHTML)) > notificationEmailMaxHTMLLength {
		return NotificationEmailPreview{}, fmt.Errorf("rendered email html cannot exceed %d bytes", notificationEmailMaxHTMLLength)
	}
	return NotificationEmailPreview{Subject: renderedSubject, HTML: renderedHTML}, nil
}
...
func notificationEmailSampleVariables(locale string) map[string]string {
	if normalizeNotificationLocale(locale) == notificationEmailLocaleChinese {
		variables := map[string]string{
			...
			"group_balance_display":           "none",
			"context_capacity_display":        "table-row",
			"predicted_requests":              "37",
			"remaining_balance_usd":           "0.795500",
			"pool_authoritative_balance_usd":  "0.500000",
			"normal_estimated_balance_usd":    "0.295500",
			...
			"api_key_requests":                "81",
			"wallet_requests":                 "145",
			"account_remaining":               "0.795500",
			"api_key_remaining":               "2.592000",
			"wallet_remaining":                "4.640000",
			"bottleneck":                      "account",
			"sample_count":                    "50",
...
var notificationEmailEventDefinitions = map[string]NotificationEmailEventInfo{
	...
	NotificationEmailEventPoolCapacityLow: {
		...
		Label:       "Group capacity low",
		Description: "Sent to active administrators when the configured request-count context or group predicted remaining USD balance falls below its threshold.",
		Category:    "admin",
		Optional:    false,
		Placeholders: append(append([]string{}, notificationEmailCommonPlaceholders...),
			...
			"alert_summary", "alert_summary_zh", "group_balance_display", "context_capacity_display",
			"predicted_requests", "remaining_balance_usd", "threshold_requests", "threshold_usd",
			"pool_authoritative_balance_usd", "normal_estimated_balance_usd", "pool_account_count", "normal_account_count",
			"participating_account_count", "skipped_account_count", "unknown_account_count", "stale_account_count", "incompatible_unit_account_count",
	...
	NotificationEmailEventAnnouncementPublished: {
		...
		Description: "Transactional notification sent when an administrator publishes an announcement.",
		Category:    "announcement",
		Optional:    false,
		Placeholders: append(append([]string{}, notificationEmailCommonPlaceholders...),
			"announcement_title", "announcement_content", "announcement_starts_at"),
	},
}
...
<table style="width:100%;border-collapse:collapse;">
  ...
  <tr><td>告警阈值</td><td>{{quota_threshold}}</td></tr>
	...
	NotificationEmailEventPoolCapacityLow: {
		notificationEmailDefaultLocale: {
			Subject: "[{{site_name}}] Group capacity low - {{group_name}}",
			HTML: notificationEmailCard("#dc2626", "Group capacity alert", `
<p>{{alert_summary}}</p>
<table style="width:100%;border-collapse:collapse;">
  ...
  <tr style="display:{{group_balance_display}}"><td>Pool authoritative USD subtotal</td><td>${{pool_authoritative_balance_usd}}</td></tr>
  <tr style="display:{{group_balance_display}}"><td>Normal estimated USD subtotal</td><td>${{normal_estimated_balance_usd}}</td></tr>
  <tr style="display:{{group_balance_display}}"><td>Participating accounts</td><td>{{participating_account_count}} (pool {{pool_account_count}} / normal {{normal_account_count}})</td></tr>
  <tr style="display:{{group_balance_display}}"><td>Skipped / Unknown accounts</td><td>{{skipped_account_count}} / {{unknown_account_count}}</td></tr>
  <tr style="display:{{group_balance_display}}"><td>Stale / Incompatible unit</td><td>{{stale_account_count}} / {{incompatible_unit_account_count}}</td></tr>
  <tr style="display:{{context_capacity_display}}"><td>Context account</td><td>{{account_name}} (#{{account_id}})</td></tr>
  <tr style="display:{{context_capacity_display}}"><td>Context API Key</td><td>{{api_key_name}} (#{{api_key_id}})</td></tr>
  <tr style="display:{{context_capacity_display}}"><td>Account / Key / Wallet balance</td><td>{{account_remaining}} / {{api_key_remaining}} / {{wallet_remaining}} USD</td></tr>
  <tr style="display:{{context_capacity_display}}"><td>Account / Key / Wallet requests</td><td>{{account_requests}} / {{api_key_requests}} / {{wallet_requests}}</td></tr>
  <tr style="display:{{context_capacity_display}}"><td>Average account / actual cost</td><td>{{avg_account_cost}} / {{avg_actual_cost}}</td></tr>
  <tr style="display:{{context_capacity_display}}"><td>Bottleneck</td><td>{{bottleneck}}</td></tr>
  <tr style="display:{{context_capacity_display}}"><td>Sample count</td><td>{{sample_count}}</td></tr>
  <tr><td>Triggered at</td><td>{{triggered_at}}</td></tr>
</table>
<p class="muted">{{prediction_disclaimer}}</p>`),
		},
		notificationEmailLocaleChinese: {
			Subject: "[{{site_name}}] 分组容量不足 - {{group_name}}",
			HTML: notificationEmailCard("#dc2626", "分组容量提醒", `
...
<table style="width:100%;border-collapse:collapse;">
  ...
  <tr style="display:{{group_balance_display}}"><td>池模式权威余额小计</td><td>${{pool_authoritative_balance_usd}}</td></tr>
  <tr style="display:{{group_balance_display}}"><td>普通账号估算余额小计</td><td>${{normal_estimated_balance_usd}}</td></tr>
  <tr style="display:{{group_balance_display}}"><td>参与账号</td><td>{{participating_account_count}}（池 {{pool_account_count}} / 普通 {{normal_account_count}}）</td></tr>
  <tr style="display:{{group_balance_display}}"><td>跳过 / 未知账号</td><td>{{skipped_account_count}} / {{unknown_account_count}}</td></tr>
  <tr style="display:{{group_balance_display}}"><td>过期快照 / 单位不兼容</td><td>{{stale_account_count}} / {{incompatible_unit_account_count}}</td></tr>
  <tr style="display:{{context_capacity_display}}"><td>请求上下文账户</td><td>{{account_name}} (#{{account_id}})</td></tr>
  <tr style="display:{{context_capacity_display}}"><td>请求上下文 API Key</td><td>{{api_key_name}} (#{{api_key_id}})</td></tr>
  <tr style="display:{{context_capacity_display}}"><td>账户 / Key / 用户余额</td><td>{{account_remaining}} / {{api_key_remaining}} / {{wallet_remaining}} USD</td></tr>
  <tr style="display:{{context_capacity_display}}"><td>账户 / Key / 用户请求容量</td><td>{{account_requests}} / {{api_key_requests}} / {{wallet_requests}}</td></tr>
  <tr style="display:{{context_capacity_display}}"><td>账户 / 用户平均成本</td><td>{{avg_account_cost}} / {{avg_actual_cost}}</td></tr>
  <tr style="display:{{context_capacity_display}}"><td>瓶颈</td><td>{{bottleneck}}</td></tr>
  <tr style="display:{{context_capacity_display}}"><td>样本数</td><td>{{sample_count}}</td></tr>
  <tr><td>触发时间</td><td>{{triggered_at}}</td></tr>
</table>
<p class="muted">{{prediction_disclaimer_zh}}</p>`),
		},
	},
	NotificationEmailEventContentModerationViolation: {
		notificationEmailDefaultLocale: {
...
<div style="white-space:pre-wrap">{{announcement_content}}</div>
<p class="muted">Published at: {{announcement_starts_at}}</p>`),
		},
		notificationEmailLocaleChinese: {
			Subject: "[{{site_name}}] {{announcement_title}}",
			HTML: notificationEmailCard("#2563eb", "平台公告", `
<p>{{recipient_name}}，您好：</p>
<h2>{{announcement_title}}</h2>
<div style="white-space:pre-wrap">{{announcement_content}}</div>
<p class="muted">发布时间：{{announcement_starts_at}}</p>`),
		},
	},
}
