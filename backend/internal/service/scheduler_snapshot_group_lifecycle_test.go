...
func TestSchedulerGroupLifecycleActiveReopensAndRebuildsAllCurrentBuckets(t *testing.T) {
	...
	accounts.beforeLoad = func() {
		...
		require.Equal(t, schedulerCanonicalBucketCountForTest(), tokenCount, "all reopen tokens must be prepared before the first account query")
	}
	svc := newGroupLifecycleTestService(cache, accounts, groups, config.RunModeStandard)
	seen := make(map[batchSeenKey]struct{})
	...
	expectedOpenAICalls := 1
	if GroupPlatformSupportsMixedScheduling(PlatformOpenAI) {
		expectedOpenAICalls++
	}
	require.Equal(t, expectedOpenAICalls, accounts.platformCallCount(PlatformOpenAI))
	for _, bucket := range current {
	...
	require.Len(t, reopenHeld, schedulerCanonicalBucketCountForTest())
	for _, held := range reopenHeld {
		require.True(t, held)
	}
	lockTTLs, unlockCalls := cache.lockStats()
	require.Len(t, lockTTLs, schedulerCanonicalBucketCountForTest())
	for _, ttl := range lockTTLs {
		require.Equal(t, 30*time.Second, ttl)
	}
	require.Equal(t, schedulerCanonicalBucketCountForTest(), unlockCalls)
	requireLifecycleSeen(t, seen, groupID)
}
...
func TestSchedulerGroupLifecycleEpochPreventsABA(t *testing.T) {
	...
	canonicalCount := schedulerCanonicalBucketCountForTest()
