...
func (s *GeminiMessagesCompatService) ForwardNative(ctx context.Context, c *gin.Context, account *Account, originalModel string, action string, stream bool, body []byte) (*ForwardResult, error) {
	...
	if resp.StatusCode >= 400 {
		...
		writeUpstreamServiceModelError(c, modelerror.ProtocolGeminiGenerateContent, resp.StatusCode, respBody, upstreamMsg, originalModel)
		if upstreamMsg == "" {
...
func sanitizeUpstreamErrorMessage(msg string) string {
	...
	msg = sensitiveBearerTokenRegex.ReplaceAllString(msg, `$1***`)
	return sensitiveInlineCredentialRegex.ReplaceAllString(msg, `$1***`)
}
