...
func registerQQBotAdminRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	qqBot := admin.Group("/qqbot")
	{
		...
		qqBot.GET("/onebot/config", h.QQBot.GetOneBotConfig)
		qqBot.PUT("/onebot/config", h.QQBot.UpdateOneBotConfig)
		qqBot.POST("/onebot/probe", h.QQBot.ProbeOneBot)
		qqBot.GET("/onebot/runtime", h.QQBot.GetOneBotRuntime)
...
func registerContentModerationRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	risk := admin.Group("/risk-control")
	{
		...
		risk.POST("/cyber-abuse/test", h.Admin.ContentModeration.TestCyberAbuse)
...
func registerUserManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	users := admin.Group("/users")
	{
		...
		users.GET("/:id/group-config", h.Admin.User.GetGroupConfig)
		users.PUT("/:id/group-config", h.Admin.User.UpdateGroupConfig)
...
func registerGroupRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	groups := admin.Group("/groups")
	{
		...
		groups.GET("/predicted-capacity-summary", h.Admin.Group.GetPredictedCapacitySummary)
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.POST("/validate-credentials", h.Admin.Account.ValidateCredentials)
