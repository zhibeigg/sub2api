...
const poolCapacitySamplesIndexMigration = "192_add_usage_logs_pool_capacity_samples_index_notx.sql"
const poolCapacitySamplesIndex = "idx_usage_logs_pool_capacity_samples"
...
var migrationChecksumCompatibilityRules = map[string]migrationChecksumCompatibilityRule{
	...
	// 199 在 v0.88.101 发布后发现真实库存在同用户多条 GPT 订阅，改为合并剩余额度；
	// 已执行旧版本且无冲突的实例可继续启动，未执行实例则应用修正版。
	"199_migrate_gpt_subscription_to_standard_group.sql":      newMigrationChecksumCompatibilityRule("a6630fb8073838cf533d22414c8881b9459bdf201a8bd3e6a7d31977c81dc409", "a2f2a829a56c603a0c51d67c6416fd448cbfe9eb97980ba0ea868d482cb9f935"),
}
...
func prepareNonTransactionalMigration(ctx context.Context, db migrationConnection, name string) error {
	...
	case poolCapacitySamplesIndexMigration:
		return dropInvalidIndexIfPresent(ctx, db, poolCapacitySamplesIndex)
	default:
		return nil
	}
}
