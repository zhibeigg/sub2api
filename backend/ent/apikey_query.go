...
// QueryBoundGroups chains the current query on the "bound_groups" edge.
func (_q *APIKeyQuery) QueryBoundGroups() *GroupQuery {
	query := (&GroupClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *APIKeyQuery) Clone() *APIKeyQuery {
	...
	return &APIKeyQuery{
		...
		withBoundGroups:  _q.withBoundGroups.Clone(),
...
// WithBoundGroups tells the query-builder to eager-load the nodes that are connected to
// the "bound_groups" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *APIKeyQuery) WithBoundGroups(opts ...func(*GroupQuery)) *APIKeyQuery {
	query := (&GroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withBoundGroups = query
	return _q
}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withBoundGroups; query != nil {
		if err := _q.loadBoundGroups(ctx, query, nodes,
			func(n *APIKey) { n.Edges.BoundGroups = []*Group{} },
			func(n *APIKey, e *Group) { n.Edges.BoundGroups = append(n.Edges.BoundGroups, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *APIKeyQuery) loadBoundGroups(ctx context.Context, query *GroupQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *Group)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(apikey.BoundGroupsTable)
		s.Join(joinT).On(s.C(group.FieldID), joinT.C(apikey.BoundGroupsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(apikey.BoundGroupsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(apikey.BoundGroupsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "bound_groups" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
