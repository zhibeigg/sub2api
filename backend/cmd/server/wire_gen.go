...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	weComOAuthCache := repository.NewWeComOAuthCache(redisClient)
	weComOAuthClient := service.ProvideWeComOAuthClient(weComOAuthCache)
	weChatPaymentOAuthService := service.NewWeChatPaymentOAuthService(client, paymentConfigService, settingService, weComOAuthClient)
	authHandler := handler.ProvideAuthHandler(configConfig, authService, userService, settingService, promoService, redeemService, totpService, userAttributeService, weChatPaymentOAuthService)
	...
	adobeTokenProvider := service.ProvideAdobeTokenProvider(accountRepository, proxyRepository, configConfig)
	compositeTokenCacheInvalidator := service.ProvideCompositeTokenCacheInvalidator(geminiTokenCache, adobeTokenProvider)
	...
	playgroundService := service.NewPlaygroundService(apiKeyService, gatewayService)
	...
	openCodeQuotaService := service.NewOpenCodeQuotaService(httpUpstream, proxyRepository, configConfig)
	usageCache := service.NewUsageCache()
	accountCapacityService := service.NewAccountCapacityService(httpUpstream, configConfig, tlsFingerprintProfileService)
	cursorGatewayService := service.NewCursorGatewayService(httpUpstream, proxyRepository, tlsFingerprintProfileService, redisClient, configConfig)
	cursorDashboardAuthService := service.NewCursorDashboardAuthService(accountRepository, cursorGatewayService, oAuthRefreshAPI, configConfig)
	kiroOAuthService := service.ProvideKiroOAuthService(proxyRepository)
	...
	announcementEmailRepository := repository.NewAnnouncementEmailRepository(db)
	announcementEmailService := service.NewAnnouncementEmailService(announcementEmailRepository, emailService, notificationEmailService, configConfig)
	adminAnnouncementHandler := admin.ProvideAnnouncementHandler(announcementService, announcementEmailService)
	...
	cursorDashboardAuthHandler := admin.NewCursorDashboardAuthHandler(cursorDashboardAuthService)
	proxyHandler := admin.NewProxyHandler(adminService)
	adminRedeemHandler := admin.NewRedeemHandler(adminService, redeemService)
	promoHandler := admin.NewPromoHandler(promoService)
	registry := payment.ProvideRegistry()
	defaultLoadBalancer := payment.ProvideDefaultLoadBalancer(client, encryptionKey)
	paymentService := service.ProvidePaymentService(client, registry, defaultLoadBalancer, redeemService, subscriptionService, paymentConfigService, userRepository, groupRepository, affiliateService, notificationEmailService, weChatPaymentOAuthService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, kiroOAuthHandler, cursorDashboardAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService)
	kiroGatewayService := service.NewKiroGatewayService(accountRepository, proxyRepository, kiroTokenProvider)
	...
	gatewayHandler := handler.ProvideGatewayHandler(gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, cursorGatewayService, kiroGatewayService, openCodeGatewayService, userService, subscriptionService, concurrencyService, billingCacheService, usageService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, userMessageQueueService, configConfig, settingService, coordinator)
	openAIImageUploadTempService := service.ProvideOpenAIImageUploadTempService(configConfig)
	openAIGatewayHandler := handler.ProvideOpenAIGatewayHandler(openAIGatewayService, concurrencyService, billingCacheService, apiKeyService, subscriptionService, openAIImageUploadTempService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, grokQuotaService, configConfig, coordinator)
	adobeVideoTaskStore := service.ProvideAdobeVideoTaskStore(redisClient, configConfig)
	adobeFireflyClientFactory := service.NewAdobeFireflyClientFactory(configConfig)
	adobeFireflyAdapter := service.NewAdobeFireflyAdapter(adobeFireflyClientFactory, adobeTokenProvider)
	adobeVideoService := service.NewAdobeVideoService(adobeFireflyAdapter, adobeVideoTaskStore, openAIGatewayService, accountRepository, apiKeyRepository, userRepository, userSubscriptionRepository)
	adobeMediaHandler := handler.NewAdobeMediaHandler(adobeVideoTaskStore, adobeVideoService, openAIGatewayService, billingCacheService, apiKeyService, opsService, adobeFireflyAdapter, contentModerationService, subscriptionService, concurrencyService, configConfig)
	...
	cursorDashboardMaintenanceService := service.ProvideCursorDashboardMaintenanceService(accountRepository, cursorDashboardAuthService, cursorGatewayService, configConfig)
	...
	poolCapacityAlertService := service.ProvidePoolCapacityAlertService(poolCapacityAlertRepository, usageLogRepository, groupRepository, accountRepository, accountCapacityService, notificationEmailService, qqBotService, configConfig)
