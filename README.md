...
- **Pool-Mode Account Capacity Alerts** - Groups can opt in to predictions for the final selected `pool_mode` account. The last 50 successfully billed requests in the final group estimate remaining account, API key, and wallet request capacity; a strict result below 50 creates durable administrator-email and current-QQBot C2C deliveries without blocking gateway billing ([Design, configuration, and admin API guide](docs/POOL_CAPACITY_ALERTS.md))
