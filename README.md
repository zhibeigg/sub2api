...
- **Dual Subscription Plan Modes** - A `subscription` plan binds exactly one subscription group and uses that group's native limits. A `standard_quota` plan binds one or more standard balance groups and defines shared daily, weekly, or monthly plan limits plus an optional concurrency limit. Quota and concurrency values are copied into the subscription-instance snapshot at purchase or assignment and later plan edits are not retroactive. An active plan takes priority over balance billing on its standard groups; public groups return to balance billing after expiration
