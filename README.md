...
- **Built-in Dual-Transport QQBot Operations** - Keeps the official Tencent BotGo webhook/message runtime and adds an independent SnowLuma OneBot v11 reverse-WebSocket transport for regular QQ group messages and real `group_increase` welcomes. Both transports share commands and binding rules while using isolated Redis streams, per-user command cooldowns, encrypted credentials, hot enable/disable, health diagnostics, allowlisted welcome policies, and a dedicated admin workspace. Unchanged OneBot configuration refreshes preserve the active reverse WebSocket; only real runtime changes trigger a handoff. Administrators can separately enable automatic friend approval and automatic user join approval for allowlisted groups; both are disabled by default, group invitations are never accepted automatically, and request flags remain confined to the encrypted queue. Allowlisted groups can directly use `/bind name@example.com` (the email remains visible in group history), while verification links are delivered only to the email. OneBot accepts only direct loopback or private-network reverse WS peers and requires Bearer authentication, the expected `X-Self-ID`, one active connection, correlated action/echo timeouts, and no public action port. The `/check` command renders a channel-availability PNG; one-time mailbox links, transport-scoped identity binding, a single first-bind balance grant, redeem ledger records, and audit history remain transactionally protected ([Integration, API, security, migration, and rollback guide](docs/QQBOT_INTEGRATION.md))
...
- **Dual Subscription Plan Modes** - A `subscription` plan binds exactly one subscription group and uses that group's native limits. A `standard_quota` plan binds one or more standard balance groups and defines shared daily, weekly, or monthly plan limits plus an optional concurrency limit. Daily, weekly, and monthly quota windows roll every 24 hours, 7 days, and 30 days from the subscription purchase/effective timestamp instead of resetting at calendar midnight. Quota and concurrency values are copied into the subscription-instance snapshot at purchase or assignment and later plan edits are not retroactive. An active plan takes priority over balance billing on its standard groups; public groups return to balance billing after expiration
