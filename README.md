...
- **Pool-Mode Account Capacity Alerts** - Groups can opt in to predictions for the final selected `pool_mode` account. The service combines verified upstream balance with the last 50 successfully billed requests, keeps local account quotas only as a safety cap, and continues using post-billing API Key quota and wallet state. A strict result below 50 creates durable administrator-email and current-QQBot C2C deliveries without blocking gateway billing; stale or unverifiable upstream data never changes the alert episode ([Design, configuration, and admin API guide](docs/POOL_CAPACITY_ALERTS.md))
