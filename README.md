...
- Discover supported model IDs and parameters with `GET /v1/models`; the admin account form starts with a 40-model offline fallback catalog, can merge synchronized upstream IDs into the whitelist, and lets Cursor accounts opt in to Anthropic `/v1/messages` group scheduling.
