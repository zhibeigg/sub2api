...
Supports two subscription plan modes: one native subscription group using its own limits, or one or more standard balance groups sharing plan-level daily/weekly/monthly limits, with administrator assignment and independent purchase visibility controls. The API Keys page includes browser-side latency checks for multiple endpoints, automatic refresh every 3 seconds, node details, and the built-in `www.pokeapi.top` backup route; when payment is enabled, users can jump to top-up directly from the header balance. The Model Square distinguishes token, per-request, per-image, and per-video billing, shows backend-computed effective multipliers plus configured image/video price tiers, and uses an independently scrolling two-column filter rail beside the model results on desktop. Model cards show the model brand rather than the internal account platform; clicking a card opens a responsive drawer listing the real platform-specific endpoints, channels, groups, billing modes, current multipliers, and multiplier-adjusted prices. Administrators can enable Model Square and Available Channels independently; see [Model Square switches and API](docs/MODEL_SQUARE.md).
...
- **Multi-Account Management** - Support multiple upstream account types (OAuth, API Key, service account); native integration for Anthropic, OpenAI, Gemini, Antigravity, Grok, Kiro (AWS CodeWhisperer, serving Claude models), Adobe Firefly, Cursor IDE Chat / Cloud Agent, and the independent `platform=opencode` OpenCode Go API Key platform with Messages, Chat Completions, and Responses entrances. Groups declare multi-value `endpoint_protocols`, keep `quota_platform` for stable quota/pricing attribution, and can explicitly bind validated cross-provider accounts behind the default-off `gateway.cross_provider_compatibility_enabled` rollout switch. With `gateway.group_endpoint_routing_enabled=true`, scheduler snapshots and database fallback use endpoint-protocol buckets; disabling it restores legacy provider buckets for emergency rollback ([Integration Guide](docs/OPENCODE_GO_INTEGRATION.md)).
- **OpenAI Plan-Aware Account Filtering** - The admin account list filters K12, Free, Plus, Pro, or unset plans on the server, so pagination, totals, auto-refresh, filtered bulk edits, and filtered exports share one scope. Use `plan_type` on `GET /api/v1/admin/accounts` and `GET /api/v1/admin/accounts/data`, or inside `POST /api/v1/admin/accounts/bulk-update` filters; `__unset__` matches a missing/blank effective plan, and linked shadow accounts inherit the parent plan for filtering.
...
- **Priority Multi-Group API Keys** - Bind one key to multiple model groups, search and reorder them by failover priority, display effective rate multipliers, and explicitly target a bound group with `X-Sub2API-Group-ID`. Playground discovery now unions every bound group's model-square channel catalog, filters it by current routing capability, and deduplicates by model name so account-only cross-platform mappings are not exposed as extra choices ([API Guide](docs/API_KEY_MULTI_GROUP.md))
- **Per-User Standard Group Restrictions** - Administrators can keep inherited access or enable a standard-group allowlist, including an explicit deny-all empty list. Exclusive-group grants, subscription-managed groups, and user-specific rate multipliers remain independent; single-group keys return `403 GROUP_NOT_ALLOWED`, while multi-group keys skip restricted candidates and fail over to the next binding ([Admin and API Guide](docs/ADMIN_USER_GROUP_RESTRICTIONS.md))
- **Multimodal Playground Workbench** - Chat supports image and text/code attachments, temperature / top_p / max_tokens / reasoning controls, model-gated web search and code execution, plus SSRF-protected public URL fetching. Image mode provides a responsive controls/canvas/history workspace with reference-image editing; uploaded references are request-scoped temporary files and are removed after every terminal path, with startup and periodic cleanup for crash leftovers. Browser conversations and recent image results are not persisted with API key values
...
- **Unified Account Capacity in Usage Windows** - The admin account list shows verified upstream balance for pool-mode Bearer API Key custom upstreams, stale/unsupported/unknown states without treating them as zero, and clearly labeled remaining-request estimates for non-pool official windows or local quotas. Sub2API-compatible upstreams use the strongly identified `GET /v1/usage` contract ([API contract](docs/API_KEY_USAGE.md))
- **Group Capacity Visibility & Alerts** - Group Management shows each current-page group's USD balance plus a configurable remaining-capacity estimate: the legacy historical-request algorithm or a fixed per-image account-capacity cost, reported as remaining requests or images while preserving complete, known-lower-bound, unlimited, and insufficient-data states. Capacity display configuration is independent of alerts: the legacy 50-sample `predicted_requests` alert and group-level `remaining_balance_usd` alert keep their existing semantics, and only values strictly below the selected alert threshold notify administrators ([Design, configuration, and admin API guide](docs/POOL_CAPACITY_ALERTS.md))
- **Smart Scheduling** - Intelligent account selection with sticky sessions
- **Concurrency Control** - Per-user and per-account concurrency limits, plus an optional per-subscription-instance pool for `standard_quota` plans; all standard groups in that instance share the pool and all three limits remain cumulative
...
- **Built-in Single-Transport QQBot Operations** - Administrators explicitly choose either the official Tencent BotGo webhook/message runtime or SnowLuma OneBot v11 reverse-WebSocket transport. Credentials for both options remain encrypted for later switching, but only the selected runtime can accept traffic, run workers, or send messages. The transports retain isolated Redis streams, transport-scoped cooldowns, health diagnostics, allowlisted welcome policies, and a dedicated admin workspace. Group-originated output always replies in the same group and never opens a private conversation with the sender. OneBot sends exactly one opening message only after the actual `friend_add` event, and then accepts private commands only from `private/friend` conversations; temporary and unknown private sessions are ignored. OneBot accepts only direct loopback/private-network reverse WS peers with Bearer authentication and the expected `X-Self-ID`; it supports correlated action/echo timeouts and no public action port. Automatic friend and allowlisted group-join approval remain disabled by default, group invitations are never auto-accepted, and request flags remain confined to the encrypted queue. Account binding, first-bind grants, ledger records, and audit history remain transactionally protected ([Integration, API, security, migration, and rollback guide](docs/QQBOT_INTEGRATION.md))
...
- **Dual Subscription Plan Modes** - A `subscription` plan binds exactly one subscription group and uses that group's native limits. A `standard_quota` plan binds one or more standard balance groups and defines shared daily, weekly, or monthly plan limits plus an optional concurrency limit. GPT/OpenAI native subscription plans are migrated to the `GPT 稳定分组 无限制` standard group while retaining each plan and user's quota snapshot, usage, and validity; users with multiple active legacy GPT subscriptions have their remaining quota combined into one target subscription with the latest expiration retained. Their requests use that group's current billing multipliers. Daily, weekly, and monthly quota windows roll every 24 hours, 7 days, and 30 days from the subscription purchase/effective timestamp instead of resetting at calendar midnight. Quota and concurrency values are copied into the subscription-instance snapshot at purchase or assignment and later plan edits are not retroactive. An active plan takes priority over balance billing on its standard groups; public groups return to balance billing after expiration
...
Model gateway errors support safe English and Chinese presentation. Clients select a language with `Accept-Language`; missing or unsupported values fall back to `gateway.model_error_default_locale` (`en` or `zh`, default `zh`). Generic internal errors and frontend last-resort messages also default to Chinese, while clients can still explicitly request English. Root-level Vue render/setup/lifecycle failures are contained by a recoverable error boundary with retry and home actions, and bootstrap failures render a safe DOM fallback without exposing exception details. Every client-visible model error is branded with `[PokeAPI]` and includes stable `X-PokeAPI-Error-Code`, `X-PokeAPI-Request-ID`, and `Content-Language` response headers.
...
- Anthropic `/v1/messages` user image blocks with `source.type=base64` are decoded and sent through Cursor's native inline-image fields. Up to 20 images are accepted, capped at 5 MiB each and 6 MiB combined; over-limit payloads return HTTP `413`. Remote image URLs, invalid Base64, and unsupported file/audio blocks return HTTP `400 invalid_request_error` without switching or penalizing another account; image requests remain on Agent RPC in `auto` mode.
- `auto` may fall back to Cloud Agent only for ordinary text requests that do not carry client tools, tool continuations, images, or Responses continuation state, when the downstream response has not been committed and a configured replay-safe error is matched. Tool- and image-capable requests remain on Agent RPC/failover instead of silently becoming repository-less Cloud Agent text answers.
...
See the [Cursor integration guide](docs/CURSOR_INTEGRATION.md) for transport modes, authentication, streaming semantics, model discovery, Cloud Agents, operational limits, and billing separation.
