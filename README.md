...
- **Built-in Payment System** - Supports EasyPay V1 MD5 and Rainbow EasyPay 2.0 RSA-SHA256 through one versioned `easypay` provider, including Alipay, WeChat Pay, and upstream-enabled QQ Pay, alongside direct Alipay/WeChat Pay and Stripe. Direct WeChat Pay instances declare Native/H5/JSAPI capabilities independently, route safely by client context, and block unauthorized modes locally. The frontend exposes three unified methods with administrator-selected sources; first-top-up promo bonuses, order attribution, net recharge totals, and filtered CSV exports remain built in, with no separate payment service needed ([Configuration Guide](docs/PAYMENT.md))
