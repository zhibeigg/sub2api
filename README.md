...
- The admin account list shows Sub2API-local request/token/cost statistics, cache-write and cache-read tokens, and configured local daily/weekly/total quota bars. A manual refresh validates the current API Key through `/v1/me`; normal list loading does not fan out upstream probes.
...
- Cursor plan usage, model usage, Cloud Agent execution, and on-demand overage remain Cursor-side charges. Sub2API uses a Cursor-specific local price snapshot for supported explicit model IDs, with channel pricing taking precedence; local usage and balances must not be presented as official Cursor credits.
