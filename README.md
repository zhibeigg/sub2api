...
- **Cursor Cloud Agents API (Beta)** - Uses `https://api.cursor.com` with user or agent-scoped service-account API Keys, `/v1/me` credential checks, `/v1/models` model discovery, a 40-model offline fallback catalog with Cursor-specific local token pricing, and repository-backed or temporary no-repository Agents, while keeping Cursor-side plan/on-demand charges separate from Sub2API billing ([Integration Guide](docs/CURSOR_INTEGRATION.md))
