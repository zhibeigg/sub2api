...
- **Group Capacity Visibility & Alerts** - Group Management shows a compact predicted remaining column with USD balance and request capacity for the current page, distinguishing complete estimates, known lower bounds, unlimited capacity, and insufficient data without treating unknown accounts as zero. Each group can also keep the legacy 50-sample `predicted_requests` alert or use `remaining_balance_usd` as a predicted group total: authoritative USD balances from pool-mode accounts plus estimated USD balances from regular accounts. Any successful charge in the group can trigger a USD recomputation; only values strictly below the threshold notify administrators ([Design, configuration, and admin API guide](docs/POOL_CAPACITY_ALERTS.md))
...
- **Built-in QQBot Operations** - Runs the official Tencent BotGo webhook and message runtime inside Sub2API, with encrypted credentials, hot enable/disable, health diagnostics, welcome policies, binding records, and a dedicated admin workspace. The `/check` command renders a channel-availability PNG: allowlisted groups/channels can use it directly, while C2C requires an `active` binding; the entry is opt-in, requests are limited to once per 30 seconds, and both `channel_check_enabled` and `channel_monitor_enabled` must be enabled. Results use short-lived HMAC-signed HTTPS image URLs, Tencent-compatible group/C2C `file_info` + `msg_type=7` or channel image URLs, safe text fallback, and redacted diagnostics. One-time mailbox links, atomic OpenID binding, a single first-bind balance grant, redeem ledger records, and audit history remain transactionally protected ([Integration, API, security, migration, and rollback guide](docs/QQBOT_INTEGRATION.md))
