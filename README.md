...
- **Dual Subscription Plan Modes** - A `subscription` plan binds exactly one subscription group and uses that group's native limits. A `standard_quota` plan binds one or more standard balance groups and defines shared daily, weekly, or monthly plan limits plus an optional concurrency limit. GPT/OpenAI native subscription plans are migrated to the `GPT 稳定分组 无限制` standard group while retaining each plan and user's quota snapshot, usage, and validity; their requests use that group's current billing multipliers. Daily, weekly, and monthly quota windows roll every 24 hours, 7 days, and 30 days from the subscription purchase/effective timestamp instead of resetting at calendar midnight. Quota and concurrency values are copied into the subscription-instance snapshot at purchase or assignment and later plan edits are not retroactive. An active plan takes priority over balance billing on its standard groups; public groups return to balance billing after expiration
