...
- **Built-in Single-Transport QQBot Operations** - Administrators explicitly choose either the official Tencent BotGo webhook/message runtime or SnowLuma OneBot v11 reverse-WebSocket transport. Credentials for both options remain encrypted for later switching, but only the selected runtime can accept traffic, run workers, or send messages. The transports retain isolated Redis streams, transport-scoped cooldowns, health diagnostics, allowlisted welcome policies, and a dedicated admin workspace. OneBot accepts only direct loopback/private-network reverse WS peers with Bearer authentication and the expected `X-Self-ID`; it supports correlated action/echo timeouts and no public action port. Automatic friend and allowlisted group-join approval remain disabled by default, group invitations are never auto-accepted, and request flags remain confined to the encrypted queue. Account binding, first-bind grants, ledger records, and audit history remain transactionally protected ([Integration, API, security, migration, and rollback guide](docs/QQBOT_INTEGRATION.md))
