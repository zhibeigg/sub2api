...
- `auto` may fall back to Cloud Agent only for ordinary text requests that do not carry client tools, tool continuations, or Responses continuation state, when the downstream response has not been committed and a configured replay-safe error is matched. Tool-capable requests remain on Agent RPC/failover instead of silently becoming repository-less Cloud Agent text answers.
