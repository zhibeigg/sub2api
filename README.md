...
- **Pool-Mode Account Capacity Alerts** - Each group can enable one alert metric for the final selected `pool_mode` account: predicted remaining requests with a custom integer threshold, or authoritative remaining USD balance with a custom dollar threshold. Request mode keeps the 50-sample cost prediction; USD mode evaluates immediately and never fabricates money from request counts or average cost. Strictly-below comparisons create durable administrator-email and current-QQBot C2C deliveries, while stale, unknown, non-authoritative, or incompatible-unit snapshots never change the alert episode ([Design, configuration, and admin API guide](docs/POOL_CAPACITY_ALERTS.md))
