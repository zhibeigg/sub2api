...
- **Cursor Cloud Agents API (Beta)** - Uses `https://api.cursor.com` with user or agent-scoped service-account API Keys, `/v1/me` credential checks and manual refresh probes, `/v1/models` model discovery, local daily/weekly/total usage windows, cache-write/cache-read token accounting, a 40-model offline fallback catalog with Cursor-specific local pricing, and repository-backed or temporary no-repository Agents, while keeping Cursor-side plan/on-demand charges separate from Sub2API billing ([Integration Guide](docs/CURSOR_INTEGRATION.md))
...
- Cursor accounts may additionally store desktop-login `dashboard_access_token` / `dashboard_refresh_token` credentials. When configured, a forced refresh calls the Dashboard Connect RPC on `api2.cursor.sh` and displays the official `Total / First-party / API` plan meters, included spend, and billing period used by Cursor's Spending page. A 401 triggers token refresh and encrypted persistence. This endpoint is undocumented; failures keep the last snapshot and never hide local usage.
