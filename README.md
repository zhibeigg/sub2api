...
- **Group Capacity Alerts** - Each group can keep the legacy 50-sample `predicted_requests` alert or use `remaining_balance_usd` as a predicted group total: authoritative USD balances from pool-mode accounts plus estimated USD balances from regular accounts. Any successful charge in the group can trigger a USD recomputation; only values strictly below the threshold notify administrators, while unknown, stale, unit-incompatible, or otherwise incomplete data never counts as zero or changes the group alert state ([Design, configuration, and admin API guide](docs/POOL_CAPACITY_ALERTS.md))
