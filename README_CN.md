...
- **内置 QQBot 运营中心** - 在 Sub2API 进程内运行腾讯官方 BotGo Webhook 与消息 Runtime，通过加密凭据、热启停、健康诊断、欢迎策略、绑定记录和独立后台页面统一管理；`/check` 可生成渠道可用性 PNG，白名单群/频道无需绑定即可使用，C2C 私聊必须存在 `active` 绑定；入口默认关闭，每个身份 30 秒限流，且 `channel_check_enabled` 与 `channel_monitor_enabled` 必须同时开启。图片使用短期 HMAC 签名 HTTPS URL，群/C2C 按腾讯 `file_info` + `msg_type=7` 发送，频道使用 image URL，失败时安全回退文本且不暴露敏感信息。邮箱一次性验证、OpenID 原子绑定、单次余额赠送、兑换流水与完整审计继续保持事务保护（[集成/API/安全/迁移/回滚文档](docs/QQBOT_INTEGRATION.md)）
