...
- **池模式账户容量预测提醒** - 分组可选择对最终实际命中的 `pool_mode` 账户启用容量预测；系统将已验证上游真实余额与最终分组最近 50 次成功落账的均次成本结合，本地账户额度只作为更严格的安全上限，API Key 配额和用户钱包继续使用扣费后状态。严格低于 50 次时通过管理员主邮箱和当前 QQBot C2C 绑定异步提醒；`stale` 或无法验证的上游数据不会改变持久告警 episode（[设计、配置与管理员 API 文档](docs/POOL_CAPACITY_ALERTS.md)）
- **智能调度** - 智能账号选择，支持粘性会话
