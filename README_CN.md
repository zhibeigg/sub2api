...
支持两类订阅套餐：单个订阅分组沿用分组原生日/周/月限额；一个或多个标准（余额）分组共享套餐限额，并支持管理员定向分配与充值/订阅购买入口独立显隐。API 密钥页支持多端点浏览器直连测速、每 3 秒自动刷新、节点详情与 `www.pokeapi.top` 备用入口，支付启用后可从顶部余额快速进入充值页。模型广场会区分 Token、按次与按张计费，并展示用户可用分组的有效图片倍率及已配置的 1K/2K/4K 按张价格。
...
- **内置支付系统** - 同一 `easypay` 服务商通过版本字段兼容易支付 V1 MD5 与彩虹易支付 2.0 RSA-SHA256，支持支付宝、微信支付及上游已开通的 QQ 支付，并可与支付宝官方、微信官方、Stripe 并存。微信官方实例可独立声明 Native/H5/JSAPI 能力，按终端安全选路并在本地阻断未开通模式；前台统一展示三种支付方式，由管理员分别选择来源。首充加成、订单归因、净充值统计和筛选 CSV 导出保持内置，无需独立部署支付服务（[配置指南](docs/PAYMENT_CN.md)）
