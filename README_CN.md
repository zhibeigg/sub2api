...
支持两类订阅套餐：单个订阅分组沿用分组原生日/周/月限额；一个或多个标准（余额）分组共享套餐限额，并支持管理员定向分配与充值/订阅购买入口独立显隐。API 密钥页支持多端点浏览器直连测速、每 3 秒自动刷新、节点详情与 `www.pokeapi.top` 备用入口，支付启用后可从顶部余额快速进入充值页。模型广场会区分 Token、按次与按张计费，展示用户可用分组的有效图片倍率及已配置的 1K/2K/4K 按张价格；桌面端筛选栏采用双列布局，并与模型结果区独立滚动。
