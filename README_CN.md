...
- **内置支付系统** - 支持易支付 V1/V2、支付宝/微信官方直连、上游已开通的 QQ 支付与 Stripe。微信官方实例可独立启用 Native/H5/JSAPI，并以 `mp` 公众号或 `wecom` 企业微信自建应用完成 OAuth；恢复上下文签名绑定原实例，企业微信失败不会自动回退 H5。统一前台、首充加成、订单归因、统计与导出均无需独立支付服务（[配置指南](docs/PAYMENT_CN.md)）
