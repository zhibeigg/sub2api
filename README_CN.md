...
- **池模式账户容量提醒** - 每个分组可为最终实际命中的 `pool_mode` 账户开启一个生效指标：带自定义整数阈值的预计剩余请求数，或带自定义美元阈值的权威上游剩余余额。请求模式保留 50 条成功落账样本预测；金额模式首次符合条件即可评估，绝不通过请求数或均次成本伪造 USD。只有严格小于阈值才通过管理员主邮箱和当前 QQBot C2C 绑定异步提醒；`stale / unknown`、非权威或单位不兼容的快照不会改变持久告警 episode（[设计、配置与管理员 API 文档](docs/POOL_CAPACITY_ALERTS.md)）
- **智能调度** - 智能账号选择，支持粘性会话
