...
支持两类订阅套餐：单个订阅分组沿用分组原生日/周/月限额；一个或多个标准（余额）分组共享套餐限额，并支持管理员定向分配与充值/订阅购买入口独立显隐。API 密钥页支持多端点浏览器直连测速、每 3 秒自动刷新、节点详情与 `www.pokeapi.top` 备用入口，支付启用后可从顶部余额快速进入充值页。模型广场会区分 Token、按次、按图片与按视频计费，展示后端计算的有效倍率及已配置的图片/视频价格档位；桌面端筛选栏采用双列布局，并与模型结果区独立滚动。模型卡片显示模型品牌而非内部账号平台，点击卡片可打开响应式抽屉查看按平台区分的真实端点、渠道、分组、计费模式、当前倍率和倍率后价格。
