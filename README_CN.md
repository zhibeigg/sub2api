...
支持两类订阅套餐：单个订阅分组沿用分组原生日/周/月限额；一个或多个标准（余额）分组共享套餐限额，并支持管理员定向分配与充值/订阅购买入口独立显隐。API 密钥页支持多端点浏览器直连测速、每 3 秒自动刷新、节点详情与 `www.pokeapi.top` 备用入口，支付启用后可从顶部余额快速进入充值页。模型广场会区分 Token、按次、按图片与按视频计费，展示后端计算的有效倍率及已配置的图片/视频价格档位；桌面端筛选栏采用双列布局，并与模型结果区独立滚动。模型卡片显示模型品牌而非内部账号平台，点击卡片可打开响应式抽屉查看按平台区分的真实端点、渠道、分组、计费模式、当前倍率和倍率后价格。管理员可分别启停模型广场与可用渠道，详见[模型广场开关与 API 文档](docs/MODEL_SQUARE.md)。
...
- **多账号管理** - 支持多种上游账号类型（OAuth、API Key、服务账户），原生集成 Anthropic、OpenAI、Gemini、Antigravity、Grok、Kiro（AWS CodeWhisperer，提供 Claude 模型）、Adobe Firefly、Cursor IDE Chat / Cloud Agent，以及独立 `platform=opencode`、支持 Messages / Chat Completions / Responses 三入口的 OpenCode Go API Key 平台。分组以多值 `endpoint_protocols` 声明公开入口，以 `quota_platform` 稳定配额与计价归属，并可在默认关闭的 `gateway.cross_provider_compatibility_enabled` 灰度开关下显式绑定经校验的跨供应账号。部分模型映射的跨供应文本账号可安全加入兼容分组：仅映射支持的模型会选中该账号，未映射模型会自动故障转移到同组其他可用账号。启用 `gateway.group_endpoint_routing_enabled` 后，调度快照与数据库回退使用入口协议桶；紧急关闭时恢复旧平台桶（[接入文档](docs/OPENCODE_GO_INTEGRATION.md)）。
- **OpenAI 套餐账号筛选** - 管理员账号列表可在服务端筛选 K12、Free、Plus、Pro 或未设置套餐，分页、总数、自动刷新、按筛选批量编辑和按筛选导出使用同一范围。`GET /api/v1/admin/accounts`、`GET /api/v1/admin/accounts/data` 以及 `POST /api/v1/admin/accounts/bulk-update` 的 `filters` 均支持 `plan_type`；传入 `__unset__` 匹配有效套餐缺失或为空的账号，关联影子账号按母账号套餐参与筛选。
...
- **API Key 多分组优先级** - 一把密钥可绑定多个模型分组，支持搜索、拖拽排序、有效倍率展示与故障转移优先级；客户端可用 `X-Sub2API-Group-ID` 明确选择已绑定分组。练习场按模型广场的渠道目录聚合全部绑定分组的当前可路由模型，按模型名去重后展示，确保对话、对比、图片和视频不会混入账号映射中的跨平台多余模型（[API 文档](docs/API_KEY_MULTI_GROUP.md)）
- **用户级标准分组限制** - 管理员可保持继承模式或启用 standard 分组 allowlist，空 allowlist 可明确禁止全部标准分组；专属分组授权、订阅分组和用户倍率保持独立。单分组 Key 受限时返回 `403 GROUP_NOT_ALLOWED`，多分组 Key 会跳过受限候选并尝试下一绑定（[管理员与 API 文档](docs/ADMIN_USER_GROUP_RESTRICTIONS.md)）
- **多模态练习场工作台** - 对话模式支持图片与文本/代码附件、temperature / top_p / max_tokens / reasoning 参数、按模型能力启用联网搜索和代码执行，以及经过 SSRF 防护的公开 URL 网页抓取；会话仅在浏览器本地保存，可新建、重命名、切换、删除撤销并导出 Markdown 或自包含 JSON，不保存 API Key 明文
- **精确计费** - Token 级别的用量追踪和成本计算；管理员可为分组配置模型级倍率，精确规则优先、通配规则回退，仅影响用户最终扣费而不修改渠道或平台模型价格，且用户专属分组倍率仍具有最高优先级
- **用量窗口统一容量展示** - 管理员账户列表会为池模式 Bearer API Key 自定义上游显示已验证真实余额，并明确区分 `stale / unsupported / unknown`，不会把未知渲染为 0；非池账户则展示官方窗口或本地额度推算的预计剩余请求数。Sub2API 兼容上游使用强识别的 `GET /v1/usage` 契约（[API 契约](docs/API_KEY_USAGE.md)）
- **分组容量展示与提醒** - 分组管理会为当前页同时展示 USD 余额与可配置的剩余容量：可继续按历史请求成本预测剩余请求数，也可按管理员设置的每张图片固定账号容量成本预测剩余生图数，并明确区分完整估值、已知下界、无限容量和数据不足。展示算法与告警指标彼此独立；保留的 50 条样本 `predicted_requests` 告警和分组级 `remaining_balance_usd` 告警语义不变，只有严格小于对应阈值才通知管理员（[设计、配置与管理员 API 文档](docs/POOL_CAPACITY_ALERTS.md)）
- **智能调度** - 智能账号选择，支持粘性会话
...
- **内置 QQBot 双链路运营中心** - 保留腾讯官方 BotGo Webhook/消息 Runtime，并新增独立 SnowLuma OneBot v11 反向 WebSocket 链路，用于普通 QQ 群消息和真实 `group_increase` 进群欢迎。两条链路复用命令与绑定规则，但使用隔离 Redis Stream、加密凭据、热启停、健康诊断、群白名单和独立后台页面；读取到未变化的 OneBot 配置时会保留现有反向 WS，只有实际运行参数变化才会热切换。管理员可分别开启自动通过好友申请和自动通过白名单群的用户加群申请；两项默认关闭，不会自动接受机器人入群邀请，申请 flag 只保留在加密队列中。白名单群可直接发送 `/bind name@example.com` 发起绑定（邮箱原文会出现在群聊记录中），所有群内输出都会留在原群，验证链接仍只发送到该邮箱。OneBot 仅在实际 `friend_add` 后向新好友发送一次开场消息，后续仅处理 `private/friend` 好友会话中的私聊指令；临时或未知私聊会话将被忽略。OneBot 仅接受直连的回环或私网反向 WS 对端，并强制 Bearer Token、预期 `X-Self-ID`、单活动连接、action/echo 超时关联，无需开放公网动作端口。`/check` 渠道状态图、邮箱一次性验证、按机器人身份隔离的原子绑定、单次余额赠送、兑换流水与完整审计继续保持事务保护（[集成/API/安全/迁移/回滚文档](docs/QQBOT_INTEGRATION.md)）
...
- **双类型订阅套餐** - `subscription` 套餐只能绑定一个订阅分组并读取该分组原生限额；`standard_quota` 套餐可绑定一个或多个标准（余额）分组，并在套餐内设置共享日/周/月额度与可选并发限制。GPT/OpenAI 原生订阅套餐会迁移到 `GPT 稳定分组 无限制` 标准分组，保留套餐和用户的额度快照、已用额度与有效期；同一用户的多条有效旧 GPT 订阅会合并各周期剩余额度并保留最晚有效期，请求按该稳定分组当前倍率计费。日、周、月额度分别从订阅购买或生效时间起按 24 小时、7 天、30 天滚动刷新，不再按自然日 0 点重置。额度和并发在购买或分配时写入订阅实例快照，后续套餐编辑不追溯；标准分组有有效套餐时优先消耗套餐额度，套餐过期后公开分组恢复余额计费
...
在 **管理后台 → 公告管理** 创建或编辑最终状态为 active 的公告时，可选择“发送给所有有效用户邮箱”。开关默认关闭；启用后，HTTP 请求只负责在同一事务中保存公告并创建持久化广播任务，后台 Worker 会按已验证主邮箱异步发送并展示进度。群发范围不跟随公告 Targeting，每条公告最多创建一个任务，后续编辑不会重新群发。

`config.yaml` 的 `announcement_email` 段控制功能开关、收件人/投递批次、并发、轮询、租约、重试和发送超时。SMTP 仍在现有邮件设置中配置。带 `send_email=true` 的创建/更新与失败重试请求必须发送稳定的 `Idempotency-Key`。完整收件规则、状态、配置、管理员 API 和 SMTP exactly-once 边界见 [公告邮件设计与 API 文档](docs/ANNOUNCEMENT_EMAIL.md)。
...
- 管理后台“风控中心”提供数据库持久化的内容审核与 Cyber Abuse Guard 配置；Cyber Abuse Guard 默认关闭，启用后独立于外部审核 API 的采样与可用性执行本地高置信度前置阻断，并提供 `/api/v1/admin/risk-control/cyber-abuse/test` 无副作用试跑接口

Cyber Abuse 试跑接口需要管理员认证，请求体为 `{"text":"待评估内容"}`；响应 `data` 包含 `matched`、`category`、`rule_id`、`confidence`、`error_code` 和 `message`。试跑不会写审计日志、发送通知、累计违规次数或封禁用户。
...
转发模式、认证、实时流语义、模型发现、Cloud Agent、运维限制与计费拆分详见 [Cursor 接入文档](docs/CURSOR_INTEGRATION.md)。
