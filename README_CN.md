...
- **内置 QQBot 运营中心** - 在 Sub2API 进程内运行腾讯官方 BotGo Webhook 与消息 Runtime，通过加密凭据、热启停、健康诊断、欢迎策略、绑定记录和独立后台页面统一管理；邮箱一次性验证、OpenID 原子绑定、单次余额赠送、兑换流水与完整审计继续保持事务保护，并发提交、Webhook 重投和解绑后重绑不会重复到账（[集成/API/安全/迁移/回滚文档](docs/QQBOT_INTEGRATION.md)）
