...
- **池模式账户容量预测提醒** - 分组可选择对最终实际命中的 `pool_mode` 账户启用容量预测；系统以最终分组最近 50 次成功落账的账户平均成本和用户平均实际扣费估算账户、API Key 与用户剩余请求容量，严格低于 50 次时通过管理员主邮箱和当前 QQBot C2C 绑定异步提醒，持久状态与逐收件人重试不会阻塞网关计费（[设计、配置与管理员 API 文档](docs/POOL_CAPACITY_ALERTS.md)）
- **智能调度** - 智能账号选择，支持粘性会话
