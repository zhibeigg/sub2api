...
- **分组容量展示与提醒** - 分组管理会为当前页显示紧凑的“预估剩余”列，同时展示 USD 余额和请求容量，并明确区分完整估值、已知下界、无限容量和数据不足，未知账号绝不按 0 处理。每个分组还可继续使用保留的 50 条样本 `predicted_requests` 提醒，或用 `remaining_balance_usd` 统计池模式权威 USD 余额与普通账号估算 USD 余额之和；只有严格小于阈值才通知管理员（[设计、配置与管理员 API 文档](docs/POOL_CAPACITY_ALERTS.md)）
- **智能调度** - 智能账号选择，支持粘性会话
